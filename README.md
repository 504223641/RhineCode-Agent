...
当前主线到 **C14**，以 DeepSeek Provider 为主。能力是逐章叠加的，下面每一层都仍在生效：
...
- **行为记录（trace，测试设施）**：`--trace` 开启后，把运行中「实际发生了什么」按时间顺序写成二十三类结构化事件的 JSONL——模型收到的完整请求与响应、每次权限判定走到第几层、每个工具的参数结果耗时、界面上出现过的每行文字、上下文压缩与 Skill 激活的每次变化。配一个只读 CLI 阅读器（时间线摘要 / 按类型与作用域过滤 / 展开单条）。**缺省关闭且零侵入**：不开时链路上不存在任何中间层、不写一个字节；开启也只观测、不改变任何权限判定。用来验收既有能力，以及排查那类「界面上看不出、但行为确实不对」的问题。
...
> 已登记为待改（[`docs/todo/2-subagent-cancel-semantics.md`](docs/todo/2-subagent-cancel-semantics.md)）。
...
| `run_agent` | 把子任务委派给独立上下文的子 Agent，只拿回结论。可要求 `isolation` 在独立 Git 工作目录中运行。**同样不进权限管线**，理由同上 | 否 |
...
│   ├── models.py        # 二十三类事件枚举、四种作用域、截断/脱敏/白名单纯函数
...
python -m unittest discover -s tests      # 2001 项，skipped 4
```
...
`docs/c13/acceptance/` 与 `docs/c14/acceptance/`（后者两份：逻辑层 + 真实模型）。


## 当前阶段文档

**当前主线是 C14（子 Agent 工作区隔离），文档在 [`docs/c14/`](docs/c14/README.md)**——四份
`spec` / `plan` / `task` / `checklist` + `README`（导航）+ `acceptance/`（**两份**验收记录）。

> ⚠️ 进门先读 [`docs/c14/README.md`](docs/c14/README.md) 的「开发与验收期抓出的问题」：
> 十几处**都是不报错的**——编译过、测试全绿、界面正常，只是某个行为悄悄不对。
>
> 两份验收记录都要读：`acceptance/logic-and-e2e.md` 验「代码做了它该做的」，
> `acceptance/live-model.md` 验「模型会不会那么用」。后者用真实模型跑出 7 个前者
> 没抓到的问题，其中三个**代码一行没错，是模型读到的文本让它走了岔路**。

C13（子 Agent 系统）文档在 [`docs/c13/`](docs/c13/README.md)。
...
2. **`Esc` 不会取消正在跑的子 Agent**（C14 真实模型验收实测发现）：按 `Esc` 只停主循环，子 Agent 线程继续跑到底——实测里它在 `Esc` 之后又跑了 7 轮、写文件、提交、留下一个工作区。隔离场景下不至于造成损害（写的都在工作区里），但**非隔离子 Agent 会继续往主项目根里写**。要停它现在得用 `/agents cancel all`。改法牵动 C13「委派永不阻塞」的契约，需单独决策，详见 [`docs/todo/2-subagent-cancel-semantics.md`](docs/todo/2-subagent-cancel-semantics.md)。
3. **`worktree.link` 目前一律降级为 `copy`**：软链指向工作区之外，被权限管线第②层拒绝，子 Agent 一个字节都读不到。已止血为「降级 + 明确警告」，要真正可用得动第②层边界判定，详见 [`docs/todo/5-worktree-link-sandbox.md`](docs/todo/5-worktree-link-sandbox.md)。
...
15. 记忆系统后续项：向量数据库/RAG 语义检索、团队记忆同步/跨机器共享、跨实例实时一致性、笔记自动清理与遗忘机制、各阈值可配置化、存档格式版本迁移与加密存储。
