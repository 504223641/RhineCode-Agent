...
- **行为记录（trace，测试设施）**：`--trace` 开启后，把运行中「实际发生了什么」按时间顺序写成二十七类结构化事件的 JSONL——模型收到的完整请求与响应、每次权限判定走到第几层、每个工具的参数结果耗时、界面上出现过的每行文字、上下文压缩与 Skill 激活的每次变化。配一个只读 CLI 阅读器（时间线摘要 / 按类型与作用域过滤 / 展开单条）。**缺省关闭且零侵入**：不开时链路上不存在任何中间层、不写一个字节；开启也只观测、不改变任何权限判定。用来验收既有能力，以及排查那类「界面上看不出、但行为确实不对」的问题。
...
│   ├── models.py        # 二十七类事件枚举、四种作用域、截断/脱敏/白名单纯函数
