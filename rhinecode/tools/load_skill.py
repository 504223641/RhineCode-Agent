...
class LoadSkillTool(Tool):
    ...
    **`read_only = True` 的两个后果，改动此标志前必须同时评估**：
    ...
    # 那是**对齐改造之前**的语义。改造的 F8 已经把「在哪执行」与「谁能触发」拆成
    # 正交两维：`context: fork` 的 Skill 模型**同样可以自行发起**
    # （`conversation.run_forked_for_model` 就是为它写的，`ActivationStatus.FORKED`
    # 的 docstring 里也写明「这不是失败」）。留着那句话等于让模型以为
    # 一半的 Skill 它碰不了。
    #
    # ## 为什么措辞要「有点 pushy」
    #
    # Anthropic 官方 skill-creator 的指导原话：描述该写得有点 pushy，因为
    # 「Claude 有**可测量的欠触发倾向**」。实测确认过这条——用户说
    # 「帮我做个前端页面」、清单里有前端 Skill，模型照默认做法做完、一次没加载。
    #
    # 所以这里与清单表头同口径：**命中就先调**、**用它替代默认做法**、
    # **用户不必点名**、**拿不准就调**。四句缺一不可（照 Claude Code 的
    # Skill 工具描述改写，它的原话是 "call this tool first … to follow
    # in place of your default approach"）。
    description = (
        "加载一个 Skill 的完整操作指令。**手上的任务属于清单里某个 Skill 覆盖的类型时，"
        "先调本工具**——该 Skill 的完整 SOP 会注入你的上下文，"
        "此后按它的步骤做，而不是按你自己的默认做法做。\n"
        "**用户不必明确说「用某个 Skill」**：判断依据是任务类型是否匹配，不是他有没有点名。"
        "拿不准要不要调时**倾向调用**——漏调会产出不符合本项目约定的结果（用户往往很晚才发现），"
        "多调一次只是多读几百字。\n"
        "标注「子对话」的 Skill **你同样可以自行发起**（会开一条子对话跑完，"
        "并把结论作为本次调用的结果回灌给你）。只有标注「仅用户可发起」的不行，"
        "那时会返回失败提示与应当建议用户执行的命令。\n"
        "可用的 Skill 名字见系统提示中的 Skill 清单；清单里只列了名字、没有说明的那些，"
        "名字看着可能相关就直接调来看看。重复加载同一个 Skill 是安全的（只会更新其参数）。"
    )
    parameters = {
        "type": "object",
        "properties": {
