...
class TodoWriteTool(Tool):
    ...
    description = (
        "维护你的待办清单。用户在界面上**一直看得到**它，"
        "这是他在一个多步任务进行中唯一看得到的进度来源。\n"
        "\n"
        "**什么时候用：**\n"
        "\n"
        ...
        "\n"
        "- **每次都传完整清单**，不是只传改动的那几条。"
        "你传什么，清单就变成什么——没带上的条目会消失，"
        "这就是删除一条的正常方式。\n"
        "- 状态要**如实反映实际情况**：真的正在做才写 `in_progress`。"
        "同时推进好几件事时，**允许多条同时是 `in_progress`**——"
        "不要为了看起来整齐而说假话。\n"
        "- 标题写成祈使句，一句话说清做什么"
        "（「改 login 接口」而不是「登录相关」）。\n"
        f"\n最多 {MAX_ITEMS} 条。超出会被拒绝（不会被截断），"
        "这时请把步骤合并得更粗一些再提交。"
    )
