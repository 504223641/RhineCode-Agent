...
class ConversationManager:
    ...
    def request_cancel(self) -> int:
        ...
        :returns: **本次取消没有停下来的**子 Agent 数量（仍在运行的条数）

        ## 为什么要返回这个数

        `Esc` 的真实语义是「**我不等了**」，不是「全停」——它只置位主循环的信号，
        子 Agent 线程照常跑到底。这是 C13「委派永不阻塞」契约内的行为，
        但**与用户直觉不符**：真实验收里用户按了 Esc 之后，子 Agent 又跑了 7 轮、
        写了文件、`git commit`、留下一个新的隔离工作区与分支
        （见 `docs/c14/acceptance/live-model.md` 第四节 A）。

        用户拍板的修法是**保持语义、把话说清**（方案 A）：不动契约、不动任何
        并发路径，只在按下 Esc 时告诉用户「还有几个在后台跑、怎么停」。
        方案 B（连带全停）与 C（只停 awaited 的）都要重新论证 C13 的委派契约，
        且 B 会杀掉模型明说过不等的 `background=true` 任务。

        **数字由本方法给出而不是让 TUI 自己去查**：`request_cancel` 有两个调用方
        （真人按键、端到端驱动器的 `cancel`），而驱动器的文档说它「等价于真人按 Esc」。
        把「查条数」留在调用方，两条路径迟早会分叉——而分叉处恰恰是验收依据。

        ⚠ 只读一次计数，**不碰任何子 Agent 的 `cancel_event`**：那会变成方案 B。

        副作用：置位取消信号。不推送任何消息（渲染是 TUI 的事——
        `TaskManager` 的加锁不变量要求临界区只做纯内存读写，
        跨线程调度与回调一律在锁外）。
        """
        self._cancel_event.set()
        return self.running_subagent_count()
