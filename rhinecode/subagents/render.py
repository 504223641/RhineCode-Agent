...
_INDEX_HEADER = (
    ...
    "那就是命中。",
    "",
    "⚠️ **不要「先看一眼再决定」。** 这是最容易犯的错：先 glob 一下、读两个文件"
    "「确认项目有多大」，然后判断「不大，我自己来吧」——**可你已经读了，"
    "上下文已经被占掉，委派的价值当场归零**。"
    "**判断依据是任务类型，不是项目大小**：调研类的活在你接到它的那一刻就该派出去，"
    "而不是自己探完之后再回头判断还要不要派。",
    "",
    "⚠️ **拿不准要不要委派时，倾向委派。** 两边的代价不对称："
    "该委派而没委派 = 那些文件内容从此常驻你的上下文，"
    "之后**每一轮请求都要把它们重发一遍**，很快就要压缩历史、丢掉更早的信息；"
    "多委派一次 = 多一次调用，而它只回流一段结论。",
    "",
