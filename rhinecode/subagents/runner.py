...
class SubAgentRuntime:
    ...
    :param main_mode: 取主对话当前权限档位的回调。**必须是回调而不是取值**。
        ...
        原始理由是「用户可能在子 Agent 排队期间按 `/perm` 切档，取值型会用上
        一个陈旧的档位」。auto-plan 扩展删掉了 `/perm`，而现有的两个预设**档位
        相同**，所以那个具体场景**当前不会发生了**——这里如实记下来，免得
        下一个人拿着一个已经不成立的理由去推理别的事。

        **但仍然保持回调**，理由换成更结实的一条：`cycle_preset` 依旧会往
        引擎上写档位（那句 `set_mode` 是刻意留的，见 `presets.py`），
        只是当前两个预设写进去的值恰好一样。哪天加一个档位不同的预设，
        取值型会**立刻**把「陈旧档位」这个 bug 带回来，而它不报错、
        只表现为某个子 Agent 的权限档莫名其妙是上一次的。
        用回调的成本是零，换回取值型是拿一个真实风险去换零收益。
