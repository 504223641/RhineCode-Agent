...
# 什么时候开始」的结果，比明确失败更难处理（c13 spec F20）。
#
# c15 由 3 提到 5：协作场景下「一个 Lead 拆四条任务派四个人」是常见形态，
# 3 个太紧，模型会被迫把本可以并行的活串起来做。
#
# ⚠ **只数「正在跑」的**。c15 的待命队员（干完活、留在场上等消息）
# **不占名额**（spec F15）——它们没有线程在跑、不发 API 请求，
# 占名额会让「留人待命」变成一种惩罚，模型学会的将是「用完就让它死掉」，
# 而那正好废掉了唤醒续跑。待命人数另有上限，见 `team/models.py`
# 的 `MAX_IDLE_MEMBERS`（那条约束的是内存，不是并发）。
MAX_CONCURRENT = 5
