...
# ⚠ **c15 的协作工具（task_create / task_list / task_get / task_update /
# send_message）刻意不在上面这张表里** —— 那正是 C15 的目的：
# spec F22 要求它们对主 Agent 与全部子 Agent **都**可见，队员才能共享看板、
# 互相说话。看到「子 Agent 拿到了五个新工具」不要当成漏改顺手补上。
#
# 这不与 C13「子 Agent 的能力只会比主对话小」冲突：协作工具不读写文件、
# 不执行命令，副作用限于改本进程内存；而队员通过它让**别人**干的每一件事，
# 仍逐个过完整的五层权限管线 + Hook 前置层。
#
# **`run_agent` 与 `load_skill` 必须继续留在表内**：C15 只让队员「能说话」，
# 没有让它们「能招人」。少了这两条，一条 `tools: [send_message, run_agent]`
# 的角色定义就能让子 Agent 拿到委派能力、无限嵌套下去。
