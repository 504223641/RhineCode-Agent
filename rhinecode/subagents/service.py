...
class SubAgentService:
    ...
    def cancel_all_for_session_switch(self) -> int:
        ...
        ⚠ **取消之外还要开新的一代**（`begin_session`）。取消只管得住「还在跑的」，
        管不住「**已经跑完、还没交付的**」——后者会在切换之后的第一次请求里被
        `take_deliverables` 取走，追加进本该空白的历史。真实模型验收撞到过：
        `/clear` 之后模型坚称上一段的子 Agent 还在跑，一次委派都没发起。

        两件事必须在**同一个方法**里做完，因为它们防的是同一件事的两半；
        分开放会让下一个新增会话切换入口的人只调其中一个（而漏掉哪个都不报错）。
