...
def render_team_brief() -> str:
    ...
    **唯一两处文本**」。C15 只做了工具描述那一处，漏了更要紧的另一处。
    ...
    ## ⚠ 措辞方向已于 2026-08-10 反转，别照着旧注释推理

    **此前这里写的是「措辞要有点 pushy」**，判断依据定成「任务之间相不相干」
    而非「活多不多」——为的是对抗上面那次欠触发观测。

    **实测把模型推到了另一个极端。** 用户反馈「一个非常简单的任务都要让子
    Agent 去做」，而欠触发与过触发**是同一个模型**（`deepseek-v4-flash`）。
    所以问题不在模型强弱，在于三处文本（本段 + 135 角色清单 + `run_agent`
    的描述）全都只写了「该组队的理由」，一句「什么时候不该」都没有。

    现在整体对齐 Claude Code：**默认不组队**，组队要有明确理由。
    完整的对照表与论证在 `subagents/render.py` 的模块 docstring 里，
    **改本段前先读那一段**。

    ⚠ **Claude Code 里根本没有与本段对应的系统提示**——它的协作能力
    （`SendMessage`）只在工具描述里一句话带过。本段之所以保留，是因为
    RhineCode 的共享清单是**独有机制**，不说模型无从知道它存在。
    因此本段的定位已从「推动」改成**「说明机制」**：讲清有什么、怎么用，
    触发条件与委派共用同一套（默认不用，用户要求或确实独立的长活才用）。
    ...
    return (
        ...
        "——整个过程不需要你逐条派发。\n"
        "\n"
        "⚠️ **默认不要组队。** 组队是委派的放大版，成本也是：每个队员都从"
        "**空白上下文冷启动**，你已经知道的背景每个人都要重新推导一遍。"
        "触发条件与单个委派相同——**用户开口要求**，"
        "或者**确实有几件互不依赖、各自都要好几轮才能做完的活**，"
        "并行能省下可观的时间。\n"
        "\n"
        "⚠️ **这些都不构成组队信号**：任务字面上分成了三条、用户说了「全部」"
        "「分别」、你觉得活有点多。**几件各自一两步就能做完的事，"
        "你自己顺序做完更快**——起三个队员的开销比那三件事本身还大。\n"
        "\n"
