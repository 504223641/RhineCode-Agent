...
def render_todo_reminder(
    ...
    **仍然 0 次**。
    ...
    ④ **把当前那条 in_progress 的标题念出来**（2026-08-18 加）。原文只说
       ...
       读多了等于没读。念出标题之后它变成一个**指名道姓的**要求，
       而那种要求很难当没看见。
    ⑤ **不许向模型提问、并且明说「别在正文里复述进度」**（2026-09-18 加）。
       ④ 的第一版把标题念出来之后写的是「**它做完了吗？**……先回答这个问题」
       ——模型于是**真的每轮当众回答一遍**。真实 trace 实录：连续八轮里有
       六轮的正文开头是「第 3 条还没做完——……」，六句话几乎一模一样，
       用户看到的就是「它卡住了在原地重复」（而它其实每轮都在正常干活）。

       ⚠ **这条与 ④ 不矛盾，两者的区别是「指名」与「提问」**：念出标题让它
       想起来那件具体的事（要保留），而问句逼它当众作答（要去掉）。把 ④
       退回泛泛的催促来「顺手」解决重复，等于把 2026-08-18 那次修复撤销了。

       ⚠ **也不要改成「隔 N 轮才提醒一次」**（Claude Code 正是那么做的：
       `TURNS_SINCE_WRITE=10` 且 `TURNS_BETWEEN_REMINDERS=10`，两个条件都满足
       才注入）。它那么做得起是因为它的模型静态提示词就吃得住，而本项目两轮
       真机验收摆着——静态提示词那条路加满三个杠杆仍是 **0 次调用**，
       每轮注入是唯一被证明有效的杠杆。**问题从来不在频率，在措辞。**
    ...
    return (
        ...
        "\n"
        "它做完了的话，**在你下一次调 `edit_file` / `write_file` / `run_command`\n"
        "之前**就调 `todo_write` 把它标成 completed、把下一条标成 in_progress\n"
        "——**每次都传完整清单**。攒到最后一起标完成等于用户全程看不到进度。\n"
        "\n"
        "还没做完就直接接着做：**不必调本工具，也不要在正文里复述进度。**\n"
        "清单的状态用户在界面上自己看得到，每轮再写一句「第 N 条还没做完」\n"
        "只是同一句话的重复，看起来像卡住了。正文要写就写这一步的新情况。"
    )
