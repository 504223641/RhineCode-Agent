...
# 自动触发余量：两条规则各算一个值，**取大的那个**。
#
# ## 规则一（小窗口）：比例 0.21，夹上限 13000
#
...
# 于是白跑一整轮判断、历史照样涨到溢出。
#
# 0.21 这个比例使 65536 附近的窗口逐字维持那次改造后的取值：
# 65536 × 0.21 = 13762 > 13000，被上限夹回 13000。
#
# ## 规则二（大窗口）：比例 0.03，不夹上限（2026-09-17 新增）
#
# 只有规则一时，窗口再大余量也停在 13000——1 000 000 的窗口下触发线落在
# **98.7%**，只剩 1.3% 的空间吸收估算误差。而本层的估算是「锚点 + 增量」的
# 近似值（`estimate.py`），**误差的绝对值随历史长度增长**：同样 1% 的相对误差，
# 在 64K 窗口上是 650 token，在 1M 窗口上是 10 000 token，已经吃掉大半个余量。
#
# 0.03 取自 Claude Code：它在 1M 上下文的模型上约 967K 触发自动压缩（≈96.7%），
# 其余模型按 200K 边界；Codex 那边是 `auto_compact_token_limit` = 窗口 × 90%。
# 本项目取 97%，落在两家之间偏晚的一侧。
#
# ⚠ **这一条的意义在 2026-09-17 之后显著变重**：c8 第一层存盘那天被整层删除，
# 摘要从此是**唯一**会在逼近窗口时出手的机制，前面没有任何东西替它挡一挡了。
#
# ## ⚠ 上限与下限的方向是相反的，别顺手对齐
#
# `MARGIN_CAP` 管的是规则一，语义是「小窗口别留太多余量」——余量是预留出来
# **不用**的空间，留多了就是浪费，所以它该有上限。
# 而「多大的窗口该留多少」必须**随窗口等比放大**，所以规则二刻意**没有上限**。
# 给规则二加一个上限，等于把「余量不跟着窗口走」这个缺陷原样造回来，
# 只是换了个更大的数字——那正是第一层存盘被删掉的原因。
MARGIN_RATIO: float = 0.21
MARGIN_CAP: int = 13000
# 大窗口侧的比例（对齐 Claude Code 的 ~97% 触发点）。**刻意不夹上限**，见上。
LARGE_WINDOW_MARGIN_RATIO: float = 0.03
...
def _derive_margin(window: int) -> int:
    ...
    两条规则取大：小窗口由「比例 0.21 夹 13000」主导，大窗口由「比例 0.03」主导。
    两者在 window ≈ 433 000 处交接，**该点以下逐字维持改造前的取值**。

    副作用：无（纯函数）。
    """
    small = min(int(window * MARGIN_RATIO), MARGIN_CAP)
    large = int(window * LARGE_WINDOW_MARGIN_RATIO)
    margin = max(small, large)
