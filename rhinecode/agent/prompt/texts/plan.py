...
PLAN_FULL = (
    ...
    "（不写文件、不改文件、不执行命令），当前也只开放了只读工具。\n"
    "\n"
    "按以下流程工作：\n"
    "1. 先用只读工具（读文件、查找、搜索代码等）充分调研，理解现状与需求。\n"
    "2. 若需求有不清楚、有歧义或需用户拍板的细节，用 ask_user 工具逐一提问：一次一个问题、"
    "给出若干候选项，每个候选项含 summary（一句话概述）与 detail（详细说明与取舍），"
    "并把你最推荐的放在 options 第一个。\n"
    "3. 调研与澄清完成后，用 present_plan 工具提交一份清晰的计划（plan 字段）等待审批。\n"
    "4. 用户通过 present_plan 批准后才会开放全部工具，你才能开始执行。\n"
    "\n"
