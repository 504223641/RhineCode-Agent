...
- 用户通过 present_plan 批准后，execution_phase 置真，本轮后续迭代放开全部工具开始执行；
  副作用工具照常逐个过完整权限管线，弹不弹确认面板由管线的判定结果决定
  （⚠ auto-plan 扩展之后缺省档是放行档，写文件与运行命令**不再逐个弹面板**——
  此处原写着「仍逐个确认」，那是扩展之前的行为，已随审批面板的说明一并订正）。
    ...
    def _execute(
        ...
        for tc in tool_calls:
            ...
            self._safe_emit(
                ...
                # `allow（④模式）`，读的人会以为用户切到了放行档。
                #
                # ⚠ **上面系统级工具那处刻意不加这个字段**：那七个工具落 `other`
                # 分支、`kind != "write_path"`，本层对它们恒不生效，加进去就是一个
                # 恒为假的字段——只会稀释掉 `mode_downgraded` 那条真正有信息量的
                # 标记。阅读器用 `.get()` 取值，字段缺席即假，不会出错。
        ...
        for tc in plan_blocked:
            ...
            res = ToolResult(
                ...
                output=(
                    ...
                    + (f"{hint}\n" if hint else "")
                    + "规划阶段只允许只读调研（读文件、搜索、查看结构）与向用户提问。\n"
                    "若这一步是方案的一部分，请把它写进计划、用 present_plan 提交给用户审批；"
                    "获批后你才可以执行它。不要改用别的工具绕过这一限制。"
                ),
