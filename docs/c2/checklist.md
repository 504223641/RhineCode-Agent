...
- [ ] **AC9 /exit**：输入 `/exit`，程序正常退出；**连按两次 Ctrl+C** 同样正常退出；
      **单次 Ctrl+C 不应触发退出**（第一次只给一行提示，且屏幕上有选中文本时它是复制）
      > ⚠ **本条已由 `docs/extensions/tui-display/`（F31/F32）改写。** 原文是
      > 「按 Ctrl+Q 同样正常退出；Ctrl+C 不应触发退出」。改写的是**判据形态**
      > 而不是意图：AC9 当年的理由是「Ctrl+C 用于复制场景，不应触发退出」，
      > 而「单次不退、两次才退」正是兑现它——`Ctrl+Q` 现已不再绑定退出。
      > 行为侧的护栏见 `tests/test_tui_quit.py`（空闲 / 流式运行中 / 面板挂起中
      > 各验一次单按不退），源码侧见 `tests/test_tui_keybindings.py`。
