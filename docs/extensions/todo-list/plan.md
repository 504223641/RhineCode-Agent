...
- `build_view(items, limit=5) -> Optional[TodoView]`
  ...
  取哪 5 条的规则（spec F13，验收期反转过一次）：**不排序**，
  原样保持模型给的执行顺序；超限时**窗口锚在第一条未完成的条目上**
  （末尾不足时往前收，保证窗口是满的）。被省掉的条数与其中已完成的
  条数写进 `overflow`。

  ⚠ 初版按「进行中 > 待办 > 已完成」重排、把已完成推到最后，
  真机上读起来不像计划而像按状态分的堆，用户否掉了。**别改回去。**
...
## T1 实测结论（2026-08-18）：主方案成立，三条判据全中

复刻 `HistoryView` 的**全部**设定（`border: solid` + `padding: 0 1` +
内层 `Vertical` 的 `height: auto; min-height: 100%` + `on_mount` 里的
`anchor()`），外挂一个 `dock: bottom; height: auto; border-top: tall` 的
`Static`，终端固定 **80×24**，无头驱动实测：

| 量 | A：块 2 行 · 滚到底 | B：块 2 行 · 滚到顶 | C：块 4 行 · 滚到底 |
| --- | --- | --- | --- |
| `HistoryView.region` | `(0,0,80,21)` | 同左 | 同左 |
| 待办块 `region` | `(2,17,74,3)` | **`(2,17,74,3)`** | `(2,15,74,5)` |
| `scroll_y` | 21 | 0 | 21 |
| `max_scroll_y` | 24 | 24 | **26** |
| 实际可见内容行数 | 16 | 16 | **14** |

- **判据一 · 钉住**：滚到顶与滚到底，待办块 `region.y` **都是 17**。✅
- **判据二 · 占位**：块高 3 → 5（+2）时，`max_scroll_y` 24 → 26，
  实际可见内容 16 → 14（**−2**，与块高增量精确相等）。✅
- **判据三 · 不遮挡**：内容实画区 `y 1..14`，待办块 `y 15..19`，**不重叠**。✅

### ⚠ 实测顺带发现的一条陷阱，写测试时必须知道

**`scrollable_content_region.height` 恒为 19，不扣 dock 子节点的高度。**
三种形态下它都是 19，而实际可滚动的视口只有 16 / 14 行。

因此**断言「占位」时不能用它**——用它会得出「待办块没占地方」的错误结论，
而屏幕上明明占了。正确的量是 **`max_scroll_y`**（它扣了），
或者 `内容总高 − max_scroll_y`。

这条已反映进 `task.md` 的 T22 与 `checklist.md` 第五节。

### 结论

**照主方案实施**：`TodoPane` 作为 `HistoryView` 的子节点、`dock: bottom`。
plan 里登记的**方案 B 不启用**，但保留在文档里——将来若换 Textual 版本
后这三条判据不再成立，那是现成的退路。

## T19 复核（2026-08-18）：产品代码上四条全中

T1 量的是**复刻品**，T19 用产品那一份真实的 `HistoryView` + `TodoPane` +
`RhineApp.CSS` 又量了一遍。**两次都要做**：复刻品可能漏抄某个设定，
而真机上多了 `append_system` 建出来的真实消息行与真实的 `#stage` 布局。

终端 80×24，历史区 40 条消息：

| 量 | A0：无待办 | A：块 4 行 · 滚到底 | B：块 4 行 · 滚到顶 | C：块 6 行 · 滚到底 | Z：收起 |
| --- | --- | --- | --- | --- | --- |
| `HistoryView.region` | `(0,0,80,23)` | 同左 | 同左 | 同左 | 同左 |
| 待办块 `region` | `(0,0,0,0)` | `(2,18,74,4)` | **`(2,18,74,4)`** | `(2,16,74,6)` | `(0,0,0,0)` |
| `max_scroll_y` | **19** | 23 | 23 | **25** | **19** |
| 实际可见内容行数 | 21 | 17 | 17 | **15** | 21 |

- **① 钉住**：滚到顶与滚到底，待办块 `region.y` **都是 18**。✅
- **② 占位**：块高 4 → 6（+2）时可见内容 17 → 15（**−2**，精确相等）。✅
- **③ 不遮挡**：内容实画 `y 1..15`，待办块 `y 16..21`，**不重叠**。✅
- **④ 零回归**：收起后 `HistoryView.region` 与 `max_scroll_y` **逐字回到基线**
  （23 / 19），待办块高度 0。✅ 这一条是 spec N5 的直接证据。

⚠ 仍未被这两次实测覆盖的：**框线看起来连不连续**、**颜色分不分得清**、
**拖选手感**。几何对不等于看着对——那些由 `checklist.md` 第五节的真机复核承担。
