...
⚠ **`permission/engine.py` 的判定逻辑一行都不改**，这是 N1/N2/N3 最强的保证形态
——不是"我们小心地没改坏"，是"那些函数根本不在改动清单里"。

**精确说法**（拆任务时实测出来的，比原先那句"一行不动"准）：该文件有**两处
docstring** 提到已被删除的 `/perm`（`:ivar mode:` 与 `set_mode` 的说明），
要随之改口径。**没有任何可执行语句被修改**——`_decide_core`、`_apply_protected`、
`decide`、`derive` 四个函数逐字不变。

留着不改是更坏的选择：一份指向不存在的命令的文档，比一次诚实的注释改动危险
——下一个人会照着它去找那个命令。
