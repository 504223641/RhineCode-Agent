...
- [ ] **三处失准的描述已改**（spec F18）
      验证：C13 安全边界第 ④ 条已改写为「缺省即 auto，子 Agent 能写文件、
      能跑命令」，**并给出了保持旧行为的办法**（角色声明更严的档位）。

      > ⚠ **本条的判据在实施期改过一次，理由记下来免得下次被当成放水。**
      > 原判据是 `grep -n "缺省配置下子 Agent 实际只能做只读" CLAUDE.md`
      > **无输出**。实际写的时候把那句原文**作为「已被推翻的原文」引用保留**了
      > （标着「原文（2026-08-14 前成立）」），因为本项目通篇的做法是留下
      > 「当初为什么那样」——直接删掉等于把这段历史扔了（`docs/c11/` 保留原始
      > 四份文档是同一个理由）。
      >
      > 于是那条 grep 会命中一次，而**要求它无输出就等于要求删掉历史**。
      > 判据改成看**语义**：那句话不得作为**当前有效的陈述**出现。
      > 机器侧的等价判据是「该短语的每一次出现，上方 3 行内都有『原文』标记」：
      >
      > ```bash
      > n=$(grep -c "缺省配置下子 Agent 实际只能做只读" CLAUDE.md)
      > m=$(grep -B3 "缺省配置下子 Agent 实际只能做只读" CLAUDE.md | grep -c "原文")
      > [ "$m" -ge "$n" ] && echo OK
      > ```
      >
      > ⚠ 判的是 `m >= n` 而不是 `m == n`：`-B3` 取的是**上下文块**，块里可能
      > 有不止一行带「原文」二字（实测就是 2 比 1）。要求相等会红在一个与
      > 判据无关的地方——这与 `CLAUDE.md` 里「别用挂钟时间做判据」是同一类错误：
      > **判据必须只对它声称要判的那件事敏感**。
