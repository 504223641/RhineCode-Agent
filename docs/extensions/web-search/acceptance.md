...
> ⚠ **补记（同日晚些时候）：误伤已经修了，而且修的过程推翻了本节的成因判断。**
> 上面「① 这是一次误伤，成因是转录污染」里的**污染**二字被更细的取样证伪：
> 照那个说法构造的六条加强样本 0/96，一次都没复现；真正的形态是
> **用户要求过一件同类的危险事，之后同类动作被当成在执行那件事**。
> 上表「未在本次改动的」那三行里，「改提示词」那条最终是**做了**的
> （当时判断为「属独立评审的改动」，那个判断本身没错——它确实单独评审了一轮）。
> 完整数据见 [`docs/c16/acceptance/transcript-scope.md`](../../c16/acceptance/transcript-scope.md)。
> **判断 ② 与 ③ 不受影响**：措辞敏感依旧，死路提示那个 bug 早已修掉。
