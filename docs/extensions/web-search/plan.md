...
> 状态：**已批准**（2026-08-19，第 1 轮；两处增补已裁定，见文末）
>
...
### ④ 权限：配置模板加一段搜索规则说明（**不做加载期容错**）

`permission/config.py` 的 `_CONFIG_TEMPLATE`（首次运行生成的那份全注释模板）
加一小段，写明三层意思（spec F10a）：正确写法是不带括号的 `WebSearch` / `deny: WebSearch`；
带括号的任何写法**一律不生效且不会有警告**；想彻底关掉请用配置里的总开关。

⚠ **加载期容错（`web_fetch` F13 那套「allow 丢弃 / deny 降级 + 警告」）
在 plan 评审时明确决定不做**，因此 `permission/config.py` 的**校验函数一行都不改**，
也**不新增 `WEB_SEARCH_RULE_NAME` 常量**（既有的 `WEB_FETCH_RULE_NAME` 只服务于那个校验，
没有校验就没有它的用武之地，加一个用不到的常量只会让人以为哪里有对应逻辑）。

代价是 `deny: WebSearch(*)` 静默无效。**这段模板文字是这条已知边界的唯一防线**
——写规则的人只会看那个模板。checklist 里 AC10a 钉住它，AC10 则**正面断言
「不产生任何警告」**，使日后「顺手补个警告」的改动会当场变红、被迫走一遍评审。
...
│   └── config.py           — 修改：**只动 _CONFIG_TEMPLATE 的注释文字**，
│                              校验函数与常量一行不改（见「既有代码改动 ④」）
├── classifier/
...
两条都在 plan 评审时定了，记在这里免得日后被当成「还没想过的事」重新提出来。

### 一、`WebSearch(...)` 带括号写法的加载期容错 —— **不采纳**

**提出的问题**：`allow: WebSearch(xxx)` 不生效是对的（少放行 = 偏严），
但 **`deny: WebSearch(xxx)` 也不生效**——一个想拦住搜索的用户写下它之后，
什么都没拦住，而且没有任何提示。

**建议过的做法**（照抄 `web_fetch` F13）：写坏的 allow 整条丢弃、写坏的 deny
降级为整工具拒绝，两支各出一条警告。

**裁定：不做。** 因此：

- `permission/config.py` 的校验函数与常量**一行都不改**；
- 代价（`deny: WebSearch(*)` 静默无效）作为**已知边界**写进 spec F10 与安全边界第 8 条；
- 缓解手段是 spec F10a：**配置模板里写明只支持不带括号的写法**——写规则的人只看那份模板；
- checklist 的 AC10 **正面断言「不产生任何警告」**。这是刻意的：日后谁要「顺手补个警告」，
  那条断言会当场变红，迫使他连同 F10 的已知边界与安全边界第 8 条一起重新评审，
  而不是悄悄改掉一个被明确裁定过的取舍。

### 二、`web_fetch` 与 `web_search` 的总开关是否合并 —— **维持不合并**

保持两个独立开关（`web_fetch_enabled` 与 `search.enabled`）。理由：
「让它能查文档，但别把我的问题发给第三方」是一个完全合理的配置，合并之后表达不了。

代价是 spec F19 那条「外部不可信内容」的系统提示注入条件必须写成**两者任一启用**，
而那正是本扩展最容易漏改的一处——checklist 的 AC26 专门钉它
（关掉 `web_fetch`、只开 `web_search`，那条约束必须还在）。
