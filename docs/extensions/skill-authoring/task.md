...
| 操作 | 文件 | 职责 |
| --- | --- | --- |
| 改 | `rhinecode/skills/__init__.py` | 包 docstring 写死「六模块严格单向依赖」，加了体检就错了 |
...
| 改 | `docs/todo/2-web-search.md`、`8-p1b-unattended.md` | 重排序号为 1、2 |
...
3. 明确排序契约并写进 docstring：**按「Skill 名 → 检查项定义顺序」排序**，
   使输出稳定、可逐条断言。
4. **改 `rhinecode/skills/__init__.py` 的包 docstring**：它现在写死
   「包内**六**模块严格单向依赖：models → parser → discovery → render →
   validation → manager」，加了体检就不成立了。顺手把 `render` 与 `validation`
   改画成同层并列——两者实际互不 import，那条竖线是既有 docstring 的笔误，
   而它恰恰是「`audit` 同时依赖两者会不会成环」的判断依据。
5. 新建 `tests/test_skill_audit.py`，写一个构造 `SkillSpec` 的测试辅助
...
2. **检查 4（过宽）**，**分两支**：
   - **非 MCP**：工具名不在只读白名单内，**且**（没有参数模式 **或**
     参数模式是纯通配）→ 产出建议，给出带**具体**模式的写法范例。
     纯通配必须算过宽：实测「匹配一切」的模式与不写模式效果逐字相同，
     而作者写下它时多半以为自己已经收窄了——比裸写更值得提醒。
   - **MCP（`mcp__` 前缀）**：改按**工具名含不含通配符**判，
     含通配符才产出建议，改法是「写成具体的工具名」。
     代码注释必须写明这一支存在的理由：权限引擎对 MCP 走 `other` 分支、
     命中**要求参数模式为空**，套用非 MCP 那支会让每条 MCP 预授权都被判成过宽，
     且「加个参数模式」这个改法**会让该条预授权彻底失效**——
     一条会把用户配置改坏的建议比不给建议更差。
   代码注释还要写明**判的是翻译后的规则而不是原始声明串**——
...
4. 测试用例：
   - 声明裸的命令类 → 命中检查 4
   - 同类别带了**具体**参数模式 → 不命中
   - **同类别带了纯通配模式 → 命中**（spec AC4 明确要求）
   - **声明裸的只读类 → 不命中**（spec AC4 明确要求）
   - **声明一个具体的 MCP 工具名（无通配符）→ 不命中**
   - **声明带通配符的 MCP 工具名 → 命中**，且改法是「写成具体的工具名」
     而**不是**「加参数模式」
...
2. 正文结构：
   ...
   - 收尾硬规则：
     **一律建到项目级、不追问层级**；
     **用户要求装到用户级时如实说明本版本做不到**并给出手工做法，
     **不要尝试写入**——用户级与内置目录都在工作区外，写类判定会被
     第②层沙箱硬拒，试了只会拿一个「路径越界」去困惑用户；
     落盘前把完整内容给用户看
...
1. 写一段一次性脚本（放 scratchpad，不进仓库），扫描内置目录、
   对四个内置样板各跑一次体检。
   > 只扫内置目录时**覆盖清单天然为空**（内置是最低优先级，永远盖不到别人），
   > 所以检查 7 恒不触发。这是**正确的**，不是漏测——别为此去伪造覆盖事实。
2. 记录每个样板的建议数与内容。
...
2. 把 `2-web-search.md` 重命名为 `1-web-search.md`、
   `8-p1b-unattended.md` 重命名为 `8-p1b-unattended.md`。
3. 改两份文档内部的序号标题，**以及正文与开工 Prompt 段里的自引用**——
   陈旧引用不只在标题：`2-web-search.md` 与 `8-p1b-unattended.md` 的正文里
...
**验证：** `ls docs/todo/` 只剩 `README.md`、`1-web-search.md`、`8-p1b-unattended.md`；

```
grep -rn "1-skill-authoring\|2-web-search\|3-p1b" . --include=*.md \
  | grep -v "docs/extensions/skill-authoring/"
```

无残留引用。

> ⚠️ **必须排除本扩展自己的四份文档**：那三个文件名会永久出现在
> `task.md` 的文件清单与本任务的步骤里、`plan.md` 的文件组织里、
> `checklist.md` 的验证项里——它们是**对这次清理动作的记述**，不是陈旧引用。
> 不排除的话这条验证在任何情况下都通不过，实施者要么困惑，
> 要么去删掉 task.md 里本该保留的内容。
