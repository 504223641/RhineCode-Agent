...
| **第二批** | **B4 `mcp_add_server` 补闸 + B5 三处文案**、B1 永久放行、B2 F22 接线、**B3 fork 子对话补 `hooks`**、**B6 RHINE.md 编码** | 1 天 | 六条用户能感知的缺陷。**B4 排第一**——它是本报告里唯一一条「六层防御一层都不生效」；B3 是安全边界被绕过，只有一行实参 |
...
| **第五批** | A3 README 重写（A4 截图**已决定不做**） | 1 天 | 作品集门面 |
