...
> ✅ **2026-08-31 状态**：本报告 A–E 全部条目**均已有归宿**——修了，或已立项进 `docs/todo/`。
> 最后一批（A5 / A6 / B3 / B4 / B5 六条）由四个并行子 Agent 于当日修完，
> PR #65–#68，CI 六格全绿，合并后全量 3471/3471。
> ⚠ **本轮标题上的 🔴 曾有 6 条停在已完成的条目上**（A1 / A2 / A3 / B1 / B2 / C1，
> 正文早就写了 ✅ 而标题没改）——与 `NEXT.md` 的 F5 复选框同一个毛病，一并修正。

## A. 发布阻塞（开源前必须做，共 6 条 —— 其中 **A4 已决定不做**）

### A1 ✅ 无 LICENSE 文件

> ✅ **已做（2026-08-29，PR #55）**：MIT LICENSE，并让它真的进分发包。
...
> ✅ **已做（2026-08-30，PR #59）**：1250 行 →278 行，旧内容降级成 `docs/guide/` 下 15 份用户手册，本节列的 11 处事实错误一并清掉。
...
- **建议**：**不要修补，重写。** 现在的 README 是「开发日志式」的：第一屏就是 15 行
  能力表加每条几百字。陌生人需要的顺序是——一句话讲清是什么 →
  30 秒装上跑起来 → 然后才是深度内容。现有的 1,250 行降级为 `docs/` 里的深度文档
- **代价**：半天。但这是**作品集场景下性价比最高的一件事**

### A4 ⬜ 一个 TUI 项目，零截图 —— **已决定不做**

> **2026-08-30 用户拍板：README 不放任何图，走纯文字版。本条到此为止。**
>
> 这条**刻意保留而不是删掉**，与 `docs/todo/README.md` 的「已取消」一节、
> `docs/internals/known-issues.md` 里那些已推翻的条目同一个做法：
> 一条被静默删掉的发现，下一个人只会**重新提一遍**，而当初为什么不做的理由
> 已经没有地方查了。

- **证据**：全仓 `find -name "*.png" -o -name "*.svg"` 无命中
- **后果**：GitHub 页面上，一张界面图的说服力超过前 500 行文字。
  这是终端应用**唯一**能在几秒内证明「它真的能跑」的方式
- **原建议**：出 3 张——① 完整对话 + 工具调用 ② 权限确认面板 ③ 子 Agent 并行活动区
- **处置**：**不做。** 实现过一版（脚本从真实界面导出 SVG，三张都出来了、
  内容也核对过），随后按用户决定连同脚本与 `docs/assets/` 一并撤除。
  README 现在是纯文字版，「它真的能跑」改由 CI 徽章与测试条数承担
- **代价**：不再适用

### A5 ✅ 三个内置子 Agent 角色没被打进分发包，且静默为空

> ✅ **已做（2026-08-31，PR #67）**：package-data 补 `"rhinecode.subagents" = ["builtin/*.md"]`。
> **真打了一次包验证**（`pip wheel --no-deps`，先 `rm -rf build`）：三个 `.md` 确实进包了，
> 全部非 `.py` 文件从 5 个变 8 个。单元测试证明不了这一条——3400 多条测试全跑在源码树上。
> 护栏 `tests/test_package_data.py`（4 条）**从文件反查模式**而不是断言某一条配置存在，
> 因此下一个新增随附资源的包也会被抓到。变异实测：删掉那行 → 红并精确点名三个文件。
...
> ✅ **已做（2026-08-31，PR #67）**：「实测三组」表换成 R5 的四组表，结论从
> 「冗余保险」改成「**这一段是唯一的开关，删掉即全丢**」，并标注与已知项 #18 同型。
...
> ✅ **已做（2026-08-31，PR #65）**：补 `hooks=self._hooks` 一个实参。
> 护栏 `HookDispatchTest`（3 条）跑完整链路并真的执行一次 `run_command`，
> **一条用例里两个断言缺一不可**——「Hook 确实分发过」且「命令确实没执行」
> （只验前者的话，一个「分发了但把结论丢掉」的实现照样全绿）。
> 变异实测：删掉那个实参 → 三条全红。
>
> ⚠ **顺带核出一条新问题，已立项**（`docs/todo/5-fork-skill-delegation-semantics.md`）：
> fork 侧的 `run()` 不传 `subagent_gate`，于是分身 Skill 派出去的活**不会被等**。
...
> ✅ **已做（2026-08-31，PR #66）**：走 S1 的候选一（恢复人在回路确认），
> 新增 `kind == "launch"` 一类，落点在权限管线**第④层**，与 `url`（web_fetch F7）、
> `search`（web_search F12）两个既有例外同格。附带把 `mcp/transport.py` 的
> `{**os.environ, **self._env}` 换成 `tools/run_command.filtered_environ()`。
>
> ⚠ **落点与 S1 原文不同，理由要记住**：做成「②″式的出口收紧器」的话，
> 用户手写的 `allow: mcp_add_server` 也翻不过它，确认面板上的「永久放行」
> 会写出一条**永远不会被求值的规则**——那正是②″当初不得不砍掉该选项的原因，
> 而 `launch` 没有等价的豁免键（每一次添加都是**另一个**外部程序）。
> 放在④层则三个放行选项全部诚实，安全性由「模型改不了 `permissions.yaml`」保证。
>
> 护栏 `tests/test_perm_launch_layer.py`（20 条，判定链**从 `presets.py` 现算**）
> 与 `tests/test_mcp_env_filter.py`（5 条，**真起 stdio 子进程读它拿到的环境**）。
> **七次变异实测全部会红。**
>
> ⚠ **两条剩余项已立项**：`mcp_add_server` 仍不进 C16 分类器
> （`docs/todo/4-mcp-add-server-classifier.md`）。
...
### B5 ✅（文字部分）「MCP 工具默认每次经人在回路确认」这条承诺已经不成立

> ✅ **文字已改（2026-08-31，PR #66）**：`CLAUDE.md` 三处 + `docs/guide/mcp.md`
> + `docs/guide/permissions.md`（README 拆分后那两句已落到这两份里），
> 并在④层那份「仍会弹面板的只剩几类」清单旁**明确点破 MCP 不在其中**。
>
> ⚠ **缺口本身还在，已立项**（`docs/todo/3-mcp-server-tools-no-panel.md`）：
> Server **提供的工具**（`mcp__<server>__<tool>`）在缺省预设下仍是 `allow @ mode`。
> 本条审查项只要求改文档，已照做——**所以现在文档是诚实的，但那一半没修**。
...
### B6 ✅ 一个存成 GBK 的 `RHINE.md` 会让三层项目指令**全部**消失，而任何地方都不会说一个字

> ✅ **已做（2026-08-31，PR #68）**：R4-1 点了两处，实际修了**三处**——
> `_safe_resolve` 是第三个入口（`Path.resolve()` 在 Windows 上遇畸形路径抛的也是 `ValueError`）。
> ⚠ **三处刻意用了两种宽度**：读文件两处用窄的 `UnicodeDecodeError`（`ValueError` 是
> 编程错误最常见的载体，用它兜底等于给日后塞进同一个 `try` 的代码预留一次静默吞噬），
> `_safe_resolve` 用宽的 `ValueError`（失败形态本身不是解码）。
>
> 护栏 `tests/test_memory_encoding.py`（13 条）的核心一条断言的**不是**「坏编码那组说了话」，
> 而是「**它不再是三组对照里唯一沉默的那一个**」（比行数不比关键词——关键词断言在
> 「报告结构垮了但那几个字还在别处」时会静默通过）。**两次变异实测红的是不同的用例集合**，
> 证实「两处缺一不可」这个成对关系真的被钉住了，已登记进 `paired-maintenance`。
>
> ⚠ **顺带核出一条同型问题，已立项**（`docs/todo/2-memory-encoding-remaining.md`）：
> `manager.py` 读记忆文件的四处是同一个洞，而 `memory_index()` **每次构建系统提示都会调**。

> 本条由 [R4](02-robustness.md) 查出，完整论证（含三组对照的实跑输出）见那份文档的 **R4-1**。

- **证据**：`memory/instructions.py:105` 读 `RHINE.md` 时只 `except OSError`，
  而 **`UnicodeDecodeError` 是 `ValueError` 不是 `OSError`**。于是一个非 UTF-8 的
  `RHINE.md` 会**穿过**那层写得很好的逐层容错（每层的读取失败、`@include` 越界/成环
  本来都会进 `layer.errors`），被 `memory/manager.py:135` 的 catch-all 接住，
  连同**另外两层已经读好的内容**一起被 `LoadedInstructions(text="", layers=[])` 替换
- **后果**：`custom_instructions()` 返回空串 → 系统提示里的「自定义指令」槽位
  **整个消失**（`prompt/builder.py` 跳过空模块）。而 `layers=[]` 让 `/memory` 报告里
  那个负责说真话的循环一次都不执行——**「RHINE.md 项目指令：」后面一片空白**
- ⚠ **它比「文件不存在」更难发现**：实跑三组对照，「真的没有 RHINE.md」列出三行
  「未找到」、「`@include` 越界」列出三行并带一条「警告」，**只有「坏编码」这一组
  什么都不显示**——而它恰恰是唯一「用户的指令确实存在、却没生效」的那一组
- ⚠ **同一个 GBK 文件喂给 `permissions.yaml` 会得到一句可读的中文原因**。
  四个 YAML 加载器都会说话（它们正是 R4 题面里「已确认干净」的那批），
  **`RHINE.md` 是唯一的例外**，而它是三层配置里唯一直接决定模型行为的那份
- **触发概率不低**：主力平台是中文 Windows，记事本、旧编辑器、`cmd` 的 `>` 重定向
  默认都写 GBK/ANSI。**用户不需要做错任何事**
- **建议**：① `instructions.py:105` 的 `except OSError` 扩成 `except (OSError, ValueError)`；
  ② `manager.py:136` 的兜底**不要丢 `layers`**，并让 `startup()` 把这句话放进它的返回值
  （启动提示的既有通道）。**两处都要改**，理由见 R4-1
- **代价**：小。⚠ 但它引入一处新的成对维护点，建议连反证测试一起加
...
> ⚠ **本节已由 [R7](06-docs-drift.md) 逐条复核，并推翻了下面六处说法**——
> 凡本节与 `06-docs-drift.md` 冲突，**以后者为准**，更正清单在它的「附一」。
> 要紧的是：**D1 与 D4 不是笔误而是「计数口径没有定义」**（两个数都对，
> 得先有人裁定）；**D2 的描述已经过期**（README 已于 2026-08-30 改对，
> 现在错的是 `CLAUDE.md` 与 `docs/internals/architecture.md`）；
> **D3 的真实规模是 10 处不是 2 处**，且成因**不是「没人跑自检」**——
> 实测那条自检命令在有 2 处真悬空的树上输出 6 行、全部正确、退出码 0。
> R7 另**新查出 8 条**（D5–D12）。
...
> ⚠ **上面这条建议只对了一半，[R7](06-docs-drift.md) 已给出完整方案。**
> D2 是「N 份拷贝」，自动生成治得了；**D1 是「口径未定义」，治不了**
> ——`skills/audit.py:126-132` 是 7 个检查函数，`AdviceKind` 是 8 个成员，
> 差在 `_check_grants` 一个函数产出两类建议，**两个数都对**。
> 方案见 `06-docs-drift.md`：P1 事实登记表测试、P2 序号与链接完整性测试、P3 文档分四层。
...
| **第二批** ✅ | B1 永久放行、B2 F22 接线（PR #56）；**B3 / B4 / B5 / B6 由 2026-08-31 的并行修复补齐**（PR #65 / #66 / #68） | 已完成 | 六条用户能感知的缺陷。**B4 排第一**——它是本报告里唯一一条「六层防御一层都不生效」；B3 是安全边界被绕过，只有一行实参 |
| **第三批** | C3 + C4 两条护栏 | 1 小时 | **性价比最高**——20 行代码钉住两个「漏改一律不报错」 |
| **第四批** ✅ | A2 textual 下界 + C1 CI（PR #58）；**A5 / A6 package-data 由 PR #67 补齐** | 已完成 | 二分验证已由 R5 做完，**A2 不再需要实验，直接改**；A5 是 🔴 且只有一行 |
| **第五批** ✅ | A3 README 重写（A4 截图**已决定不做**） | 已完成 2026-08-30（PR #59） | 作品集门面 |
| **第六批** ✅ | C5–C10、D1–D4、E1/E2/E4 + 两份文档一致性护栏 | 已完成 2026-08-31（PR #60–#64） | ⚠ 四处按 R3/R6/R7 的实测结论改了做法，逐条记在 [`NEXT.md`](NEXT.md) 的 F6 一节 |
...
上面六批是**修**的顺序，不是**查**的清单。五个审查阶段**全部完成**
（阶段 0 / 1 / 2 / 3 / 4 / 5，对应 R1–R7 七项）。
...
**R4（剩余异常吞噬 + flaky）已完成**，产出追加在
[`02-robustness.md`](02-robustness.md) 的「R4」一节——121 处 `except Exception` 里
「吞掉之后做了别的事」的那 67 处逐处过完（**59 处干净，8 处「降级且无痕」**），
五条高风险用例逐条评估并**在人为拖慢 6.8~9.5 倍的机器上真跑过**。
**新查出一条 🔴（上面的 B6）与三条 🟠**，另有**一条实测复现的 flaky**
（`test_bootstrap.py` 的 `_launch_until_trace`，6 轮里 1 轮红，
且**它为排查而写的那段代码自己也会超时，把要给的证据丢掉**）。
⚠ 它**更正了 `NEXT.md` 里 R4 题面自己的五处说法**，包括那三个对不上的数字
（实际是 **115 处 / 50 处 `pass` / 剩 67 处**，不是 121 / 40 / 81）——
更正清单在它的「附一」。

**R6（可维护性交付物）已完成**，产出在
[`05-maintainability.md`](05-maintainability.md)——ruff / mypy 的**渐进落地方案**
（每一步的产出条数都是现测的：第一步 7 条、第二步 78 条）、`CONTRIBUTING.md`
完整草案、两张**如实反映强连通分量**的 mermaid 架构图（语法用 mermaid 11 的
解析器验过）。⚠ **CI 那一项不出草案**——它已随 F4 落地，那一节改成逐条核对。
**新更正了 `00-baseline.md` 的三处**：ruff 两张表**不是同一个作用域**
（高信号组只量了 `rhinecode/`，默认集含 `tests/`，于是 R6 题面里的「78 个问题」
是两个作用域相加）、ruff 0.8.4 **不从 `requires-python` 推断 `target-version`
（基线全部数字量在隐式 py38 下）、强连通分量**是 10 还是 11 取决于两个口径**
（`presets` 没被算成节点；而 `context` 之所以进环，恰恰是因为
`agent → context` 那条边**只存在于 `TYPE_CHECKING` 里**——作者为了不进环
而做的事被扫描器算成了一条真边）。另**推翻基线一处判断**：那 16 个
`union-attr` 逐条打开看过，**没有一处是今天的真 AttributeError**，
全部是「判空发生在别处、mypy 看不见」。

**R7（文档漂移的根治方案）已完成**，产出在 [`06-docs-drift.md`](06-docs-drift.md)——
D 节四条拆成**五种成因不同的病**，其中 D1 / D4 是「计数口径没有定义」、
**自动生成治不了**（⚠ F6 现在那条「D1 改为七项」的指示照做会改错）。
三个实测推翻了原诊断：官方自检命令在有 2 处真悬空的树上**输出 6 行全对、退出码 0**；
D3 真实规模是 **10 处**不是 2 处；D2 **早就有护栏**且一直是绿的，
它只管一个文件、然后在失败信息里让人「顺手同步 CLAUDE.md」。
两份测试草案**真的落进 `tests/` 跑过**，合计红出 **12 处**真问题、零误报。
**新查出 8 条漂移（D5–D12）**，并更正了本文 D 节的六处说法。

**其余剩余工作与每项的一键 Prompt 见 [`NEXT.md`](NEXT.md)**——照着从上往下做，
审查就完整了。七个 R **全部完成**
（~~R1~~ / ~~R2~~ / ~~R3~~ / ~~R4~~ / ~~R5~~ / ~~R6~~ / ~~R7~~）；
六批修复里 ~~F1~~ / ~~F2~~ / ~~F3~~ / ~~F4~~ 已完成，
**剩下最该先做的是 R6 §6 那份排好序的清单**——第①步（修 `config.py:347` 的 B904
与 `conversation.py:1736` 的 F821）只要十分钟，而它是后面几步的硬前置。
