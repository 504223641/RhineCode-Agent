...
| --- | --- | --- |
| 0 建基线 | ✅ 完成 | — |
| 1 发布阻塞项 | ✅ 完成（R5 查清，F1 / F4 修完） | ~~R5~~ → ~~F1~~ / ~~F4~~ |
| 2 正确性健壮性 | ✅ 完成（R3 + R4） | ~~R3~~、~~R4~~ |
| 3 架构冲突 | ✅ 完成（R1 交互矩阵 + R7 文档漂移） | ~~R1~~、~~R7~~ → R7 产出在 [`06-docs-drift.md`](06-docs-drift.md) |
| 4 安全复审 | ✅ 完成 | ~~R2~~ |
| 6 清单漏排的六条 | ✅ 完成（F7，2026-08-31） | A5 / A6 / B3 / B4 / B5 / B6 → PR #65–#68 |
| 5 可维护性交付 | ✅ 完成（R6） | ~~R6~~ → 产出在 [`05-maintainability.md`](05-maintainability.md)，⚠ CI 那一项已随 F4 落地，R6 改为逐条核对 |

## 建议顺序

```
R1 → R2 → R3 → R5 → F1 → F2 → F3 → F4 → R4 → R6 → F5 → R7 → F6 → F7
 ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅    ✅
...
**能并行的**（已全部完成，保留供追溯）：R1 / R2 / R4 互不重叠（分别读集成测试、读权限与安全文档、读异常处理），
...
> **已完成（2026-08-30）**，产出追加在 [`02-robustness.md`](02-robustness.md) 的
> 「R4」一节（该文件现在同时装着 R3 与 R4）。
>
> **结论：67 处里 59 处干净，8 处「降级且无痕」；flaky 五条里四条的题面前提不成立，
> 而没被怀疑的那一条真复现了。**
>
> - **🔴 R4-1：一个存成 GBK 的 `RHINE.md` 会让三层项目指令全部消失，
>   而 `/memory` 里连一行都看不到。** `UnicodeDecodeError` 是 `ValueError` 不是
>   `OSError`，于是它**穿过** `load_instructions` 那层写得很好的逐层容错，
>   被 `memory/manager.py:135` 的 catch-all 接住，连同**另外两层已读好的内容**
>   一起被 `LoadedInstructions(text="", layers=[])` 替换掉。`layers=[]` 是关键——
>   `/memory` 里那个负责说真话的循环一次都不执行。三组对照实跑：
>   「真的没有 RHINE.md」和「include 越界」都如实报告，**只有「坏编码」这一组
>   整段空白**，而它恰恰是唯一「用户的指令确实存在、却没生效」的那组。
>   同一个 GBK 文件喂给 `permissions.yaml` 会得到一句可读的中文原因——
>   **`RHINE.md` 是四个配置加载器里唯一不说话的那个。**
> - **🟠 R4-2**：`write_file` / `edit_file` 的 `open(..., "w")` 先截断后返回，
>   写入中途失败时文件已经空了，而模型收到的是「写入失败」——它会据此
>   认为原文件还在。与已知项 #3（原子写入）**不是同一条**：那条讲持久性，
>   这条讲**报告的准确性**，不做原子写入也能修，6 行。
> - **🟠 R4-3**：隔离工作区的启动清理有三层 fail-safe 全通向「什么都没发生」，
>   **而埋点被 `is_empty` 挡在门外**——界面 / `/agents` / trace 三条出口同时沉默。
>   `hooks/manager.py:385` 为同一件事写下了正确的做法（「零命中也产出这条事件」），
>   `bootstrap.py:472` 也写了（「必须说出来」），而 `:581` 在同一个文件里没说。
> - **🟠 R4-4**：`glob`/`grep` 的过滤器崩了会被报成「跳过 N 个**被权限规则拒绝**的文件」
>   ——归因给了一条根本没被求值的规则，用户会去翻 `permissions.yaml`。
> - **🟡 R4-5 / R4-6**：`pre_tool_use` 分发失败是静默 fail-open 且 `/hooks` 会说
>   「从未触发，去查你的字段名」（**本轮没找到可达的抛出点**，按后果形态定级）；
>   记忆更新成功会通知、失败不会。
> - **另有六处「看着可疑、实际干净」的写进了文档**（含理由），免得下一轮再查一遍——
>   其中 `permission/protected.py:259` 是本轮最规范的一处 fail-closed 写法。
>
> **flaky 部分**（方法：起 48 个忙循环压满 16 核，把机器拖慢 6.8~9.5 倍再连跑）：
>
> - **🔴 `test_bootstrap.py:319-364` 实测复现**：6 轮里 1 轮红，`_launch_until_trace`
>   的**三个调用点全挂**。而且失败形态比「偶尔红一下」更糟——
>   **为排查而写的 `proc.communicate(timeout=10)` 自己也超时了**，
>   那句带子进程 stderr 的 `self.fail` **一个字都没出现**。根因是
>   `terminate()` 在 `finally` 里、`communicate()` 在 `try` 里，顺序反了：
>   超时时子进程还活着（那是个 TUI），管道永不关闭。
>   **即最需要证据的那一种失败，恰恰是它唯一拿不到证据的。**
> - **四条的题面前提不成立**：`test_memory_manager.py:177` 的早返回是**同步**的
>   （线程根本没起，无竞态）；`test_team_mailbox.py:84` 与产品同源
>   （`team/` 全用 `time.time()`，`sent_at` 只用于 `strftime` 显示，
>   `monotonic` 在那里连语义都不对）；`test_subagent_gate.py:246/278` 因为
>   「循环会等子 Agent」而是确定性的。**该用 `monotonic` 而没用的地方是
>   `test_bootstrap.py:345`，不是 mailbox。**
> - **`tests/e2e/c14_scenarios.py:612` 不是测试**——不被任何 `test*.py` import，
>   不进 `discover`、从未被 CI 跑过。它的真脆弱点也不是时钟（30 天余量），
>   而是 `except OSError: pass` 吞掉 `os.utime` 失败 → 人工验收看到
>   「清理没生效」→ 去查 c14 的清理逻辑，而根因在预置代码里。
> - **两条不该存在的判据**：`test_subagent_gate.py:247` 的
>   `assertLessEqual(hits[0], len(provider.bodies))` **恒真、一次都不可能红**；
>   `:142` 的 `assertLess(elapsed, 3.0)` 写在 `wait_any` 返回**之后**，
>   真挂住时根本走不到那行——**0 的护栏价值，100% 的 flaky 面**。
>
> ⚠ **一条应该先说的背景**：这五条里的前四条**已经在 F4 的 CI 上跑过 6 个格子**，
> 而那几轮 runner 慢到把**五处**互不相干的潜伏时间假设同时照了出来，
> **这四条一处都不在其中**。本轮的负载实测与那个结果一致——
> **「被列为高风险」和「真的会红」是两回事，得真跑。**
>
> ⚠ **它更正了题面自己的五处说法**（含那三个对不上的数字：
> 实际是 115 处 / 50 `pass` / 剩 67，不是 121 / 40 / 81；
> 121 里有 6 处在 docstring 里**讲解**这个模式而不是在用它），
> 更正清单在 `02-robustness.md` 的 R4「附一」。
...
> **已完成（2026-08-30）**，产出在 [`05-maintainability.md`](05-maintainability.md)。
> **纯文档，产品代码零改动，也没往仓库根装任何配置。**
>
> ⚠ **五项里第 1 项（CI）不出草案** —— 它已随 F4 落地（PR #58），
> 那一节改成「拿 R6 的要求逐条核对已落地的那份 + 指出缺口」。
> 五条要求全部对上，另指出三处缺口：**没有 lint 格**（但它有硬前置，见下）、
> 覆盖率无人盯、`cache: pip` 没开。
>
> **其余四项的产出**：
>
> | 项 | 结论 |
> | --- | --- |
> | ruff | 三步走。**第一步（`E4/E7/E9/F/B`）实测只产出 7 条**，其中 2 条是基线早就点名的真 bug、4 条十分钟能清、1 条误报。第二步加 `BLE` + `RUF100` 产出 78 条。第三步是「永远不开」的清单（含理由） |
> | mypy | **棘轮**：全仓开检查，把今天过不了的 11 个文件列进豁免名单、只减不增。另 8 个「只错 1 条」的文件建议当场清掉 |
> | CONTRIBUTING | 完整草案。**把三条别处没有的约定放在最前面**（成对维护点 / 强制中文注释 / spec 驱动），排在「怎么装环境」之前 |
> | 架构图 | 两张 mermaid，**如实画出那个 10 包强连通分量**而不是假装分层。语法用 mermaid 11 的解析器验过 |
>
> **⚠ 四处新查出来的东西**（都推翻或更正了既有说法）：
>
> - **`00-baseline.md` 的两张 ruff 表不是同一个作用域**——高信号组只量了
>   `rhinecode/`，默认集含 `tests/`。本项题面里那句「总共只有 78 个问题」
>   就是两个作用域的数相加得来的。
> - **ruff 0.8.4 不从 `requires-python` 推断 `target-version`**，没有
>   `[tool.ruff]` 时按 py38 处理。基线全部 ruff 数字都量在这个隐式 py38 下，
>   所以照配置装上之后数字会变（B905 从 0 变 4）。
> - **强连通分量是 10 还是 11，取决于两个口径**。`presets` 没被基线算成节点；
>   而 `context` 之所以在基线那个环里，**恰恰是因为 `agent → context` 那条边
>   只存在于 `TYPE_CHECKING` 块中**——作者为了「不产生潜在导入顺序问题」
>   而做的事，被扫描器算成了一条真边。
> - **那 16 个 `union-attr` 逐条打开看过，没有一处是今天的真 AttributeError。**
>   基线写的是「最可能是真 AttributeError」，那是按错误码的一般含义推的。
>   实际 11 条是同一个变量（`ClarifyPanel._question` 的生命周期保证），
>   其余 5 条是「判空在公开入口、报错在私有实现」。
>   ⚠ 教训：**静态检查器的错误码只告诉你「形状」，告诉不了你「危不危险」。**
>
> **⚠ 落地有硬前置顺序**（写在那份文档的 §6）：先修 `config.py:347` 的 B904
> 与 `conversation.py:1736` 的 F821，**再**贴 ruff 配置、**再**加 lint job。
> 反过来做的话，CI 第一次跑就是红的，而第一天开的口子
> （`continue-on-error` 或撒 `noqa`）之后不会再被摘掉。
>
> **⚠ 一条未做的复量**：mypy 建议用 `types-PyYAML` 替代
> `--ignore-missing-imports`，而装上 stub 之后可能报出新错误。
> 本轮没有改动本机环境，所以这一步留给落地时做。
...
**产出在 [`06-docs-drift.md`](06-docs-drift.md)。**

**结论**：D 节四条**不是同一种病，是五种**，其中只有两种能靠「数字自动生成」治
——D1 与 D4 是**计数口径没有定义**（7 个检查函数 / 8 个 `AdviceKind` 成员，两个数都对），
**得先有人裁定口径才写得出断言**。⚠ **F6 现在那条「D1 改为七项」的指示照做会改错**
（`skills/models.py:100` 数的是枚举）。

三个实测结果推翻了题面的诊断：① `docs/todo/README.md:21` 那条官方自检命令
在有 2 处真悬空的树上**输出 6 行、全部正确、退出码 0**——成因不是「没人记得跑」，
是**跑了也查不出来**；② D3 的真实规模是 **10 处**不是 2 处，8 处在 `docs/todo/` 之外；
③ D2 **早就有护栏**（`tests/test_trace_models.py:42`）且一直是绿的，
它只管一个文件，然后在失败信息里写「并且同步 CLAUDE.md」——**把靠人记得搬了一层**。

方案是三件独立的事：**P1** 事实登记表测试（数字由代码现算）、
**P2** 序号与链接完整性测试（零登记）、**P3** 文档分四层并写明每层「允许落后多少」。
两份测试草案**真的落进 `tests/` 跑过**，合计红出 **12 处**真问题、零误报。
另**新查出 8 条漂移**（D5–D12），并更正了 README D 节的六处说法。

**下一步**：先做 §2.2 那两条口径裁定（一次决定），再按 §6 的五批落地。
...
>
> ⚠ **前置材料当时缺了一份，事后补齐了**：本项 `前置` 写着
> 「R6（要用它的 CI 草案）」，而做这一批时 R6 还没开工、
> `docs/review/05-maintainability.md` 不存在。CI 配置是照本文 R6 那份一键 Prompt 里
> 列的要求直接写的（矩阵 / `run_parallel` / 干净安装 / 那两个坑）。
>
> **R6 已于 2026-08-30 补做**（见上方 R6 一节），并**回过头把这份 CI 逐条核对了一遍**：
> 五条要求全部对上，另指出三处缺口（没有 lint 格 / 覆盖率无人盯 / `cache: pip` 没开）。
> 核对结论在 [`05-maintainability.md`](05-maintainability.md) 的第 1 节。
...
> **已完成（2026-08-30）**，见 PR #59（分支 `readme-rewrite`）。
>
> **结论**：README 从 1250 行的「开发日志式」压到 278 行的门面版，
> 旧内容一条没丢，整体降级成 `docs/guide/` 下的 15 份用户手册。
> 本节列的那 11 处事实错误一并清掉。
> A4（截图）按 2026-08-30 用户拍板**不做**，README 走纯文字版——
> 中途曾落地过「三张图由脚本从真实界面导出」，随后连同 `docs/assets/`
> 与生成脚本一起撤掉（commit `660fbf1`）。
>
> ⚠ **本节的复选框此前一直停在 ⬜，直到 2026-08-31 汇总时才发现**——
> 事情做完了、PR 也合了，只有台账没跟上。这正是 R7 那类「文档漂移」的
> 手工版本，记在这里供追溯。
...
**已完成（2026-08-31），拆成 5 个 PR**：#60 日志设施与崩溃兜底与包元数据（C5/C6/C9）、
#61 Provider 错误分类与主对话超时（C7/C8）、#62 三条资源与生命周期（C10-a/b/c）、
#63 ruff 两个真 bug 与 5 处多余 noqa（E1/E2/E4）、#64 文档一致性护栏与 D 系列。
全量从 3323 涨到 **3426**，每批各自绿。

⚠ **四处与本节原指示不同，都是照 R3/R6/R7 的实测结论改的**：

1. **D1 不是「改成七项」而是「改成八项」。** 口径已由用户裁定为「数 `AdviceKind`
   枚举」——两个数**都对**（7 个检查函数 / 8 个枚举成员，差在 `_check_grants`
   一个函数产出两类建议），照原指示做会把 `skills/models.py` 那句改错。
   R7 §2.2 早已点名这条。
2. **E4 不是「清掉 77 个 unused-noqa」。** R6 §2.3 把这条性质翻过来了：那 141 条
   里绝大多数标的是**没有启用的规则**（BLE001 93 条），**是一份没人认领的资产
   而不是技术债**。删掉等于把 93 处「这里是刻意的」一起扔了。本批按 R6 的
   `external` 口径只清了真正多余的 **5 处**。
3. **C6 按 R3 重写**（原报告的因果链不成立：终端不会坏，catch-all 兜不住主要形态），
   换成「读退出码」+「接管崩溃展示」两条真缺口。
4. **E1 没用 `if TYPE_CHECKING:`**（R3 的修法那样写）：那能让 ruff 与 mypy 闭嘴，
   但 `get_type_hints()` **照样 NameError**，而那正是 R3 自己列出的影响面。
   改用既有的运行期 import，零成本且真的修掉。

⚠ **另外证伪了 R7 自己的一条**：D12「`05-maintainability.md` 有 5 处坏链」不成立
——那 5 处全在**围栏代码块**里，是 `CONTRIBUTING.md` 草案正文，从仓库根出发的路径
对它而言是对的。真正的收获是护栏原稿缺了「跳过围栏代码块」这一条，已补。
`06-docs-drift.md` 的 D12 行已就地更正。
...
> **已完成（2026-08-31）**，四个子 Agent **并行**做，PR #65 / #66 / #67 / #68，
> CI 六格全绿，合并后全量 **3471/3471**（本批前基线 3426，净增 45 条护栏）。
>
> **这一批的来历本身就是个教训。** 它不在原计划里——`README.md` 的「建议处理顺序」
> 表把 B3–B6 排进第二批、A5/A6 排进第四批，而本文落成 F 系列时**把范围缩窄了**
> （F2 只写 B1/B2，F4 只写 A2/C1），**中间没有任何地方记下「为什么不做」**。
> 于是六条 🔴 就这么悬空了：既没修，也没进 `docs/internals/known-issues.md`。
> 直到 2026-08-31 汇总时逐条核对代码才发现。
>
> ⚠ **本文末尾「全部做完之后」的第 2 条判据（「每条都有归宿」）当时是不成立的，
> 而上面的进度表全是 ✅。** 判据写对了没用，得有人真的拿它去对一遍。
>
> | 条目 | 修法 | PR |
> | --- | --- | --- |
> | **B3** | fork 子对话补 `hooks=self._hooks`（一个实参） | #65 |
> | **B4** | 新增 `kind == "launch"` 接在第④层 + MCP 子进程环境变量过滤 | #66 |
> | **B5** | 三处文案改成实情（缺口本身另立项） | #66 |
> | **A5** | package-data 补 `rhinecode.subagents` + 真打包验证 | #67 |
> | **A6** | 那张「实测三组」表换成 R5 的四组表 | #67 |
> | **B6** | 三处 `except`（R4-1 只点了两处）+ `startup()` 兜底不丢 `layers` | #68 |
>
> **三条值得留下的方法论**：
>
> 1. **每个任务书里都写了「护栏必须做一次变异实测」**（撤掉修复确认真的会红）。
>    本批共 12 次，全部通过。B6 那两次红的是**不同的用例集合**——这正好证明
>    「两处缺一不可」不是说辞。
> 2. **A5 那条单元测试证明不了**，必须真打一次包：3400 多条测试全跑在源码树上，
>    **结构性地看不见「文件没进包」**。任务书里那句「真打一次包把文件清单贴出来」
>    是这一条唯一的硬证据。
> 3. ⚠ **每个任务书里都有一句「把相邻的同类代码也对照一遍」，产出率是 4 条任务
>    出 4 条新发现**（已立项 `docs/todo/2` ~ `5`）。**审查（R 系列）查的是
>    「已经知道要看哪里」的东西，而修复现场是唯一能看见「隔壁还有一份同样的代码」
>    的时刻**——那一刻上下文最全，且转瞬即逝。
>
> **B4 有一处落点与 `04-security.md` 的 S1 原文不同，理由记在 `README.md` 的 B4 一节**：
> 做成「②″式出口收紧器」会让用户手写的 `allow` 也翻不过它，
> 确认面板上的「永久放行」于是变成一个骗人的按钮。
