...
| **D12** | ~~🟡~~ **已撤销** | ~~`docs/review/05-maintainability.md` 有 5 处坏链~~ —— **F6 落地时逐处核对，这条不成立**：那 5 处**全在围栏代码块里**，是 R6 产出的 `CONTRIBUTING.md` 草案正文，而那份文件将来住在**仓库根**，从根出发的路径对它而言完全正确。改成 `../docs/…` 反而会让草案装上去之后是错的。⚠ 真正的收获是**护栏本身缺了一条**：`MarkdownLinkTest` 原稿不跳过围栏代码块，于是任何一份「在文档里贴一段将来住别处的文件」都会被误报——而那正是本项目 spec 驱动开发的常规写法（同一份文件里还贴着 ci.yml 与 pyproject 片段）。已在 `tests/test_docs_links.py` 里补上跳过逻辑 | 由 §4.4 的护栏原型实跑抓出，**F6 落地时证伪** |
