...
- **`lockfile.py`（锁原语）**——最底层，纯文件系统操作：原子创建锁（`os.open` + `O_CREAT|O_EXCL`，检查与创建一步完成）、释放、过期判定（锁内容含 PID + 时间戳）。不感知「锁的是笔记还是会话」，只提供「试着拿锁，拿不到就说不行」的原语。机制与策略分离：笔记锁和会话锁的策略（跳过 vs 拒绝载入）留在使用方。
...
- **`notes.py`（笔记与索引，纯逻辑）**——笔记文件的 frontmatter 解析/渲染、四类分类常量、索引文件的重建与 200 行/25KB 截断。不做 IO 决策，只做格式转换（对标 c8 的 `summarize.py` 纯逻辑定位）。
- **`note_updater.py`（笔记 LLM）**——笔记更新的 Prompt 常量（禁用工具、要求结构化输出）、把「本轮新增对话」渲染成请求、把 LLM 响应解析为结构化的笔记动作列表（新增/更新/删除/不动）。只产出「打算做什么」，不亲自写盘——写盘权收拢在 manager 一处，配合锁的获取释放构成完整临界区，避免「拿锁的人和写盘的人不是同一个」。
- **`manager.py`（编排者）**——`MemoryManager`：唯一持 provider 引用与副作用编排。启动时加载 RHINE.md、清理过期会话、开新档；运行中提供两个槽位的注入内容、接收每条新消息做追加写、自然停止后起后台线程跑笔记更新（拿锁 → 调 LLM → 写盘 → 更新索引 → 释放锁 → 通知界面）；对外暴露 `/memory` 报告、`/resume` 列表与载入、状态查询。

**四个单点接入**（不打散既有主流程，spec N1）：

1. `__main__.py`——解析 `--continue` 参数；`ConversationManager` 构造时创建 `MemoryManager`（所有 Provider 都建，笔记能力按工具模式门控）。
...
4. **自然停止钩子**——`ConversationManager._run` 把 Agent 事件流包一层生成器：看到 `FINISHED(COMPLETED)` 就触发 `MemoryManager` 的异步笔记更新（仅工具模式）。
...
@dataclass
class NoteAction:            # note_updater.py：LLM 决定的一个笔记动作（只是意图，不含 IO）
...
**依赖：** 无（纯 stdlib）。过期阈值由调用方传入：**笔记锁 600 秒、会话锁 600 秒**（会话锁靠心跳保鲜，见 manager）。
...
### `note_updater.py`（笔记 LLM，纯逻辑 + 请求渲染）
...
class MemoryManager:
    ...
    def on_natural_stop(history)            # 起后台线程跑笔记更新（notes_enabled 才动）
...
### ③ 笔记异步更新（on_natural_stop 内部，daemon 线程）

```
进程内 in-flight 标志已置？ → 是：跳过本轮（不排队不堆积）
起线程：
  new_msgs = history[高水位:]        # 高水位 = 上次笔记检查时的消息数，避免重复审视
...
│   ├── notes.py               # 笔记 frontmatter 与索引的解析/渲染/截断（纯逻辑）
│   ├── note_updater.py        # 笔记 LLM 的 Prompt 与响应解析（产出结构化动作）
...
├── tui/app.py                 # 改：注入笔记通知回调（call_from_thread 线程安全提示行）；
│                              #     会话锁心跳定时器
...
└── test_memory_manager.py        # 编排：假 provider 笔记流程/锁被占跳过/失败静默
```
...
| 笔记 LLM 输出 | JSON 数组（每项一个 NoteAction） | 结构化易解析；宽松解析跳过坏项，整体不可解析按「无动作」处理 |
| 笔记写盘权 | 程序写盘，LLM 只产内容与归类 | 写入路径由代码锁死在两个 memory 目录内，模型输出注入不了任意路径（spec N6） |
...
| 进程内笔记并发 | 单 in-flight 标志，占用即跳过 | 不排队不堆积；快速连发消息时最多漏记一轮，下轮补上 |
| 笔记高水位 | 记录上次笔记检查时的消息数，只审视新增段 | 避免每轮把全量历史重发给笔记 LLM（成本）且防重复记忆 |
