# C9 记忆系统（项目指令 · 会话存档 · 自动记忆）Checklist
...
- [ ] 对话给出明确偏好，自然停止后用户级 memory 出现 category 正确的 frontmatter 记忆；项目知识落项目级；期间输入不被阻塞（AC14）【手测】
- [ ] 记忆新增后索引同步多一行；>200 行索引注入时只取前 200 行（AC15）【单测 test_memory_notes / test_memory_manager】
- [ ] 记忆 LLM 调用失败：对话不受影响、`/memory` 显示最近一次更新失败（AC16）【单测假 provider 抛异常 + 手测断网佐证】
- [ ] 索引内容出现在「长期记忆」槽位（stable 通道，debug 或单测断言）；模型可 `read_file` 用户级记忆全文；其它工作区外路径仍被拦（AC17）【单测 test_memory_sandbox + 手测】
- [ ] `/memory` 输出含：RHINE.md 各层状态、include 展开情况、两级索引位置与超限标记、四类记忆数、最近更新结果、当前会话 ID 与消息数、锁状态（AC18）【手测对照】
- [ ] 记忆实际变更时界面出现低打扰提示；无变更轮次不打扰（AC19）【手测】
- [ ] anthropic/openai 配置：RHINE.md 注入与 `/resume`、`--continue` 可用；不产生记忆；`/init` 返回不支持（AC20）【手测（可用假 key 验证命令分支）】

### 并发防护

- [ ] memory 目录放新鲜锁：本轮记忆跳过、不等待；删锁后下轮恢复；正常写完锁被释放（AC21）【单测 test_memory_manager】
- [ ] 双实例：B `/resume` A 的会话被拒并提示占用；B `--continue` 顺延到下一个未锁定会话；锁改过期后 B 接管且旧锁清除（AC22）【单测 attach/startup + 手测双终端】
- [ ] 锁不可创建（只读目录）时记忆跳过不崩溃；过期残留锁不会永久堵死写入（AC23）【单测 test_memory_lockfile / test_memory_manager】
...
- [ ] **场景 4·双实例并发**：两个终端同一项目各启动一个 RhineCode → B `/resume` A 的会话被拒 → 两边各自对话，记忆与索引文件无交错损坏（用编辑器检查 frontmatter 与索引行完整）。
