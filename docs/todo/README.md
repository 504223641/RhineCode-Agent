...
在 `main` 上就先起新分支再动手。建议的分支名写在各自的 Prompt 里。
...
| 优先级 | 事项 | 复杂度 | 一句话 |
| --- | --- | --- | --- |
| [1](1-skill-authoring.md) | Skill 作者期 | 中（1–2 天） | Skill 现在**可导入但不可创作**。唯一由真实使用暴露的缺口 |
| [2](2-network-tool.md) | 网络访问工具 | 大（多天） | 第一个能主动向外发数据的工具，**权限模型要重想**，前两层防线对它无效 |
| [3](3-p1b-unattended.md) | P1b 无人值守回归 | 大（多天） | 代码不难，**难在取样方法** —— 做不好会把偏了的取样固化成回归测试 |
