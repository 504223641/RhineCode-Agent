...
> **上次重排：2026-08-14**，新增 `2`（保护路径层）插在中间，原 `2`–`6` 整体后移一位。
>
...
| [4](4-delegation-trigger-eval.md) | **委派/组队的触发率** | `delegation-trigger-eval` | 中（**本轮是测量，不是改代码**） | 触发口径已于 2026-08-10 **整个反转**（从「拿不准就委派」改成对齐 Claude Code 的「默认不委派」），但**没做真实模型复测**。⚠ 原标题是「模型不会主动组队」、原第一步是「换强模型跑对照」，**两条都作废**——欠触发与过触发出自同一个模型。与第 3 条同源，评测闭环可复用 |
...
**一组同源事项，建议成对处理**：`3 + 4` 都是**触发率**的评测问题
（第 3 条建好的闭环，第 4 条能直接复用）。⚠ 第 4 条现在是**双向**的
——既要测「该派没派」也要测「不该派却派了」，别只按欠触发那一侧设计闸门。

⚠ **`1` 排在最前是因为用户已决定它是下一件事**，不是因为它比评测那条更要紧。

⚠ **`2` 排在第二位是「安全缺口优先于评测工作」的判断，不是用户拍板的。**
它不是线上事故、也没有用户报障，触发还需要放行档（或一条宽松 allow 规则）
加一次重启——觉得该往后挪就直接重排，理由记在这里免得下一个人以为是硬约束。
...
| 丙 | **2**（perm-protected-paths） | 集中在 `permission/`（`engine.py` 新增一层 + `models.py` 的 `Layer`），外加两处随 `Layer` 走的表：`trace/reader.py` 与 `tui/widgets.py`。与甲乙都不重叠 |

**⚠ 3 与 5 不要同时做**：两者都在 `tests/e2e/` 里施工——5 要改控制通道那五个
成对维护文件（`protocol.py` / `control.py` / `host.py` / `client.py` /
`test_e2e_control.py`），3 要在这套设施上盖批量执行器。同时改会撞在 `host.py`
与 `control.py` 上，而且 3 盖楼时地基还在动。**建议 5 先做**——`send --force`
这类能力 3 的批量跑多半也用得上。
...
- ~~C15 子 Agent 协作~~ —— `c15` 分支，PR #20 已合并进 main。走完整 /spec 流程，
  ...
  「模型不会主动组队」是本清单第 4 条——⚠ 它在 2026-08-10 **翻了个面**：
