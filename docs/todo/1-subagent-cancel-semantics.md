...
>
> ✅ **语义已由用户拍板（2026-08-09）：走方案 A —— 保持现状 + 明确提示。**
> 下面「这不是显然的 bug，所以要先决策」那一节保留，是为了记住 B/C 为什么被否；
> **开工时不必再问，直接按「要改哪里」的方案 A 那条实现。**
...
语义**已经定了：方案 A**（保持现状 + 明确提示），不必再问。
B/C 会动 C13「委派永不阻塞」的对外契约，已被否决——文档里那张三方案表留着
只是为了记住「为什么不选 B/C」。

按文档「要改哪里」的方案 A 那条实现：取消时查一次 subagent_service.tasks 里
RUNNING 的条数，非零就发一条系统消息，告诉用户它们仍在后台、可用
/agents cancel all 停止。⚠ 注意 TaskManager 的既有硬不变量：加锁临界区只做
纯内存读写，消息推送必须在锁外。

护栏至少一条：提示里的条数与实际 RUNNING 数一致（含「零个在跑时不提示」的反证）。
