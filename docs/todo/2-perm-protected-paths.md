...
>
> ⚠ **2026-08-14 升级为硬前置**：第 3 条（auto/plan 两 preset）会把 auto 定为
> 缺省档，届时**写文件与跑命令都不弹面板**——本条的保护路径就成了「模型改不了
> 自己的配置」的唯一依据。原文里「触发还需要放行档」那个限定条件随之作废。

## 与第 3、4 条的关系

三条同源，出自 2026-08-14 一次「Claude Code 与 Codex 的权限模式对我们意味着
什么」的对照，**必须按 2 → 3 → 4 串行**：

| | 做什么 | 为什么是这个顺序 |
| --- | --- | --- |
| **2**（本条） | 保护路径层 | 3 会去掉那次面板，先补上拦截 |
| **3** | 只留 auto / plan 两个 preset | auto = 现有 `permissive` 档 + 本条 |
| **4** | 分类器审查 `run_command` / `web_fetch` | 盖在 3 之上，收窄命令与网络 |

⚠ 2 与 3 改**同一个文件**（`permission/engine.py`），不能并行。
**顺序颠倒的后果不是「少做一件事」而是「安全性净下降」**：
3 先做等于把唯一的拦截先拆了。
