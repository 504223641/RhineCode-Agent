...
1. **`send --force`** —— 解锁 checklist 场景 9（运行中的输入反馈）。
   现在 `DriverCore.send` 的前置检查基本上是「只有 idle 才能提交」，忙碌态下提交
   递不进产品，被拦的是驱动器**自己的**守卫而不是产品行为。

   ⚠ **2026-08-19 复核补一句**：ask-user 扩展给它开了**一条例外**——澄清面板的
   自由输入态（`app._clarify_free_text` 为真）允许在 pending 时提交，
   因为那时打字**就是**正确的应答方式。所以这里不再是「写死一个状态」，
   而是「一个状态 + 一条例外」；`--force` 做成什么形状要把那条例外一起考虑进去
   （别把它实现成第二条并列的特判，两条特判迟早分叉）。本条范围其余部分不变。
...
⚠️ 开工前必须先读 docs/todo/8-p1b-unattended.md 里「必须修正的取样方法」一节。
...
做完这条后把 docs/todo/8-p1b-unattended.md 删掉，并重排 docs/todo/ 下其余文档的序号。
```
