...
它们服务 C2 起的全部已完成能力与未来所有阶段的验收，专治那类
「界面上看不出、但行为确实不对」的问题。

放在 `docs/c11/` 底下只是因为它们在 C11 期间开发——**位置只反映开发时间，
不反映归属**。请不要在任何 spec 里把它们写成某个章节的一部分。

> 它们在 C12（Hook 系统）的验收里被大量使用：`--trace` 的两类新事件
> （`hook_dispatch` / `hook_execute`）与 `tests/e2e/` 的六个端到端场景，
> 还用真实模型跑出了一个**复合命令绕过拦截规则**的真实缺陷。
> 这正是当初立这套设施的理由。
