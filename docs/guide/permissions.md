...
- MCP 远端 Server 是外部程序、不可信：MCP 工具一律视为非只读，且**缺省预设 `auto` 下
  每次调用都会弹确认面板**（`remote` 类，第④层）。同样弹面板的还有 `mcp_add_server`
  ——它会写配置并立刻拉起一个任意本地命令（`launch` 类）。**两者是同一件事的两半**：
  一个拦「把外部程序拉进来」，一个拦「它带进来的工具此后每次被调用」。

  ⚠ **这句话一度不成立。** 老说法是「默认模式下每次调用都经人在回路确认」，
  说的是 `PermissionMode.DEFAULT`，而缺省预设 `auto` 是放行档——那段时间里
  MCP 工具其实是直接放行、零提示零面板的。要**更严**仍是写 `deny` 规则或给子 Agent
  角色声明更严的 `permission_mode`；**嫌吵**则点面板上的「永久放行」，或自己写
  `allow: mcp__<server>__*`（③层命中即短路④层，那条规则下次启动真的生效）。stdio 子进程的行为不受路径沙箱约束（与 `run_command` 同属已知边界），但它**不再继承含密钥的环境变量**（复用命令工具那份过滤；`mcp.yaml` 里显式写下的 `env` 仍照常送达）；`mcp.yaml` 的 `env`/`headers` 可能含密钥，勿提交真实值。
