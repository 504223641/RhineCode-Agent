...
- **权限**：MCP 工具一律视为非只读。⚠ **但「默认权限模式下每次调用都经人在回路确认」这句老说法已经不成立**：它说的是 `PermissionMode.DEFAULT`，而缺省预设是 `auto`（放行档）。**实情是：缺省预设 `auto` 下 MCP 工具直接放行，零提示零面板。**要逐次确认，写 `deny` 规则或给子 Agent 角色声明更严的 `permission_mode`；反过来想一次放行整个 Server 用 `allow: mcp__<server>__*`（见下）。若工具名被规范化，请以 `/mcp` 显示的注册名写规则。
