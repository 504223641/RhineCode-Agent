...
窗口大小由 `config.yaml` 的可选字段 `context_window` 决定（默认 **1000000**，照 DeepSeek V4 全系的实际窗口；缺省/非法回退默认）。⚠ 换用窗口更小的模型时必须跟着调小，否则会一直压不动、直到服务端报超长。其余阈值（存盘 4K/16K、保留 10K、余量 13K 等）为模块内常量，当前不可配（YAGNI）。
