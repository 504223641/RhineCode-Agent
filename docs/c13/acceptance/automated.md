...
> ⚠ **本报告跑于两次设计修订之前**。修订后全量测试是 **1763 项通过、skipped 4**，
> 且新增了 21 项闸门测试（`test_subagent_gate.py`）覆盖「同一次运行内交付」
> 与「多委派并行」两条。下面表格里涉及 AC17（前台/后台）与 AC4（启动提示）
> 的行已过期，其余判据不受影响——见 [`../README.md`](../README.md)。
