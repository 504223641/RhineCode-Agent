...
| MCP 客户端 | `test_mcp_*.py`、`test_mcp_auto_config.py`、`test_perm_other_glob.py`、**`test_perm_launch_layer.py`**、**`test_perm_remote_layer.py`** | 两层配置合并与 `${VAR}` 展开、JSON-RPC 消息构造与响应分类、stdio 三步会话与按 id 配对（起真实子进程）、stderr drain 防阻塞、非法远端名规范化但仍调原名、`CallToolResult→ToolResult` 转换、单 Server 失败隔离、运行时重载、URL/NPM 自动解析、YAML 安全写入、Windows `npx.cmd`、`other` 分支 fnmatch 通配；**两组「MCP 不再零面板」的权限护栏**——`launch`（添加 Server，B4/S1）与 `remote`（Server 提供的工具此后每次被调用，S2）各钉住「缺省预设下的实际判定不是 ALLOW」，且**判定链一律从 `PRESET_AXES[DEFAULT_PRESET]` 现算、绝不硬编码档位名**（两条洞的成因逐字都是「换缺省档时没人回来看这条承诺」）。`remote` 那组另有三条不可简化的：走**真实 `MCPTool` 构造路径**取 kind（手写标志的话「那行声明被删掉」照样绿）、`rule_name`/`specifier` 必须逐字等同 `other` 分支（本次修复的代价必须为零，c7 AC9 的 `allow: mcp__server__*` 一个字不用改）、以及 `read_only` 必须保持 False（只读简化分支排在④层**之前**，哪天改成信远端的 `readOnlyHint` 会把整层静默跳过——而那正是本次产品决策拒绝过的方案） | HTTP 传输与真实 Server 端到端（`docs/c7/checklist.md`） |
| 上下文管理 | `test_context_*.py` | 近似估算（无锚点/有锚点/越界兜底）、**不到摘要触发线就一个字不动历史**（含「用量到窗口一半也不动」的反证）、摘要纯逻辑（边界 snap 到 user 不拆 tool 对、草稿丢弃、重构结构）、编排（摘要成功失效锚点、连续失败熔断与复位、`manual_compact` 无阈值）、**保留区与余量随窗口缩放**（含 64K 零回归与小窗口端到端判据） | 真实 LLM 摘要与 TUI 渲染 5 场景（`docs/c8/checklist.md`） |
...
- **`test_agent_continuation.py`（21 条，2026-09-18 新增）** —— 检查点上的续跑判定。
  三条必须同时成立：**不传判定器时行为逐字不变**（零回归的落点）、**问不出来
  不许当成「继续」**（判定器一坏就变成永远继续，正是这条线要防的状态）、
  **连续放行有次数上限**（判定器看得到工具输出、可以被投毒，这是唯一的结构性兜底）。
  另有一组 `SeparateFromSecurityClassifierTest` 钉住「它不是 c16 那个分类器」——
  除了「不持有熔断器」的结构断言，还直接观察 `CircuitBreaker` 的状态，
  确认让续跑判定连续失败十次也**一动不动**（只断言前者挡不住「拿一个全局单例来记」）。
  十一次变异实测全部抓到，⚠ **其中两次值得记**：
  ① 「去掉连续放行上限」那次**测试跑不完（无限循环）**——那本身就是这条上限
  要防的形态，最有说服力的一次变异；变异脚本因此必须给每次跑加超时，
  否则它会挂在那里。⚠ 连带的教训：**变异脚本被中途杀掉会把变异留在源码里**，
  这次就留下了一个 `if False:`，靠 `git diff` 才发现——杀进程之后必须核一遍工作区。
  ② 「解析改成看包含」第一版**没抓到**——用例把 `STOP` 与 `CONTINUE` 写在了
  不同的行上，而 `head` 只取第一行，按包含判也照样对。要放在**同一行**里才验得到。
- **`test_agent_spinning.py`（24 条，2026-09-18 新增）** —— 原地打转检测。
  分两半：纯逻辑（指纹与计数）与接线（循环真的会停、且停在正确的原因上）。
  ⚠ **「不该触发」那侧的用例比「该触发」的多，是刻意的**：漏判的代价是多烧
  几轮（用户看得见、会来喊停），**误伤的代价是一次正常任务被无理由掐断**，
  而用户完全看不出原因。四条反证全部取自真实 trace：量→改→复测的排查节奏 /
  改完代码再跑同一条测试 / 分页读同一个文件 / 第二次重复不许触发。
  另有一条 `StopReasonIsWiredEverywhereTest` **遍历整个枚举**钉住三张文案表，
  下一个新增的停止原因自动被覆盖。
  十一次变异实测全部抓到，⚠ **其中两次是先漏了再补上的**，都值得记：
  ① 「一轮里只记头一个调用」那次没抓到——因为用例把卡住的调用写在了**第一位**，
  排第二位才验得到；② 「指纹去掉分隔符」那次没抓到——参数走 `json.dumps`，
  字符串会被加引号，于是 `("ab","c")` 与 `("a","bc")` 这种「看起来会撞」的样本
  其实撞不上，要用**数字**才构造得出真的碰撞。
...
也成立。因此这里的反证（同一条命令上也跑一次朴素写法）不是冗余，
删掉等于把整层防护变成无人看守。
...
⚠ **反证的判据换过一次（2026-08-30，装 CI 之后 Linux 上跑出来的）。**
原写法是「朴素写法必须**慢**」（耗时 > `THRESHOLD`），而那句话**只在 Windows
上成立**：
...
同一个缺陷在两个平台上的**症状不一样**——Windows 上「又慢又漏」，Linux 上
「快但漏」——而耗时只是 Windows 那一侧的症状。CI 第一次跑时三个 Linux 格子
全红在这一条上，**而产品行为在两个平台上都是对的**（同文件另外三条在 Linux
上全绿：立刻返回、进程树真的死了）。

现在的形式是两条性质的组合，平台无关且比原来更严：

- **正面**：`run_shell_captured` 必须**同时**满足「立刻返回」与「没有泄漏」
- **反证**：朴素写法**至少违反其中一条**（刻意写成析取——两个平台上被违反的
  不是同一条，钉死任何单独一条都会在另一个平台上红，而那时红的是判据不是产品）

⚠ **这是一处成对维护点**：`CLAUDE.md` 测试一节此前把 `THRESHOLD=4.0` 列进
「明确不要动」，理由正是那条已被证伪的判据，两处不能单改。
⚠ `CHILD_SLEEP=6` 仍然不能缩小——标记文件的判定完全靠它给出的那个
「本该还活着」的窗口。
⚠ **变异实测做过**：把 `run_shell_captured` 换回朴素写法，本条当场红
（Windows 上两条断言都响）——没有这一步，「改判据」与「把判据改成永远成立」
在测试结果上看不出区别。
...
两份**不测产品行为、只测文档**的用例，跟着 `discover` / `run_parallel` / CI 一起跑。
它们治的是一类反复发生的漂移：**同一个事实活在 N 个地方，没有一处是「源」**
——写它的人每次只改手上那一份，不是不细心，是没有任何东西告诉他还有五份。

| 文件 | 钉住什么 | 事实源 |
| --- | --- | --- |
| `tests/test_docs_facts.py` | 文档里的计数 == 代码现算值（trace 事件类数 / Hook 事件数 / Skill 体检项数），外加 trace **作用域名字清单** | `len(list(SomeEnum))`，**当场从代码算，绝不硬编码** |
| `tests/test_docs_links.py` | `docs/todo/` 序号引用不过期、相对链接能解析、扩展目录全部进索引 | `ls docs/todo/`、文件系统本身 |

**落地当天红出 12 处真问题、零误报**，全部已修（D1/D2/D3/D5/D9/D11）。

⚠ **四条设计要点，每条都对应一次实测教训：**

1. **期望值当场从代码算**（与 `run_parallel` 同构）。硬编码期望值的护栏只能发现
   「代码变了」，发现不了「文档没跟上」——而后者才是要治的。
2. **「登记了却一处都没匹配上」必须单独断言。** 文案改写（哪怕只是加一对加粗星号）
   会让正则不再匹配，于是那个文件「零处需要检查」——**全绿，而文件是错的**。
   R7 原型实测正是这么漏掉整个文件的。
3. **名字清单也要比，不只比个数。** 只比个数挡不住「加一个删一个」（计数纹丝不动
   而内容全错），也挡不住名字写错——架构文档曾写作用域叫 `notes`，而代码里那个
   常量叫 `memory`，同时漏了 `classifier`：**个数与名字同时错**。
4. **`docs/review/` 整个排除，围栏代码块整块跳过。** 前者是审查台账，它的职责就是
   **逐字引用**过期的引用作为证据（不排除的话本轮的报告自己被红 16 次）；后者是因为
   代码块里常常贴着「将来住别处的文件」的草案，从仓库根出发的路径对它们是**对的**
   ——F6 落地时正是靠这一条证伪了 R7 自己记下的「5 处坏链」。

⚠ **明确不接的三样**（判据是 **变更频率 ÷ 拷贝数**，不是「能不能算出来」）：
**测试条数**（每次加用例都会变，接了等于每个 PR 都要顺手改文档，而人对噪声门禁的
标准反应是放宽断言——已改成「不写具体数」）；**`docs/c*/` 与 `docs/extensions/*/`
里的数字**（验收当时的快照，与现状不符是**正确的**）；**「每章新增几类」这种子集
说法**（它们本来就该跟着那一章冻结，而且与总数说法在文本层面长得一模一样，
全仓扫描会大量误报——**一个会误报的门禁的结局是被整体关掉**）。
