...
| 记忆系统 | `test_memory_*.py` | 锁原语（原子互斥/过期接管/touch 保鲜）、RHINE.md 三层加载与 `@include`（嵌套上限/防环/越界拦截/围栏代码块）、记忆 frontmatter 往返与索引双截断、会话存档（惰性建档/容错载入丢组/锁标记/过期清理）、编排（记忆请求 `tools=None`/锁被占跳过/高水位增量/`--continue` 顺延被锁会话）、沙箱只读白名单（读放行而写仍拒） | 真实 LLM 记忆质量与 TUI 6 场景（`docs/c9/checklist.md`） |
...
> ⚠ **2026-08-14 改过它的等待方式**：原先是 `thread.join(timeout=10)`，
> 而队员干完之后回到待命、**线程本来就不会退出**，那 10 秒每次都白等满
> （两条合计 20 秒，占当时全套件的 8%）。更要紧的是它连「跑完没有」都没验——
> `join` 超时不抛，后面的断言直接读文件。现在等的是「记录里出现第二条
> `subagent_end`」，**与断言同源**，不存在「等的东西和判的东西不是一回事」的缝。
> 这与上面那条「不要用挂钟时间做判据」是同一类：**等一个固定时长，
> 等于把判据交给机器的快慢。**

> ### ⚠️ 不要用挂钟时间做判据（2026-08-10 已修）
>
> `test_subagent_e2e.py` 里两条用挂钟时间做判据的用例曾在全量并发下**偶发失败**、
> 单跑必过（实测 `AssertionError: 0.203 not less than 0.12`）。
>
> 问题不是它红，**是它偶尔红**——那会训练所有人忽略失败，包括忽略它某天开始
> 报告的真问题。登记那轮改造期间三次要停下来确认「这条是我改坏的吗」。
>
...
> **修法（三件事，缺一不可）**：
>
> 1. **假 Provider 的子 Agent 卡在 `threading.Event` 上**，由用例显式放行，
>    判据改成「主对话返回时子 Agent 还没终态」——一个顺序事实，与机器快慢无关。
>    闸门带一个 5 秒兜底：若产品退化成「委派阻塞主对话」，三方互等本会**死锁**，
>    有了它就变成**确定性失败**。⚠ 挂住的测试比失败的测试难查得多——
>    它没有失败信息，只有一个超时的 CI。
> 2. **`_settle` 超时改成明确失败，不再静默 `return`。** 这是第二条用例
>    `AssertionError: 0 != 1` 的真正病根：等待函数悄悄放弃，把「子 Agent 压根
>    没跑完」伪装成了「交付逻辑坏了」。**一个悄悄放弃的等待函数，
>    等于把「没发生」谎报成「发生了但结果不对」**，与「观测设施绝不能撒谎」同源。
> 3. **反证写成常驻用例**（`ForegroundE2ETest.test_foreground_waits_for_the_subagent`）：
>    同一判据、相反结论。少了它，一个「委派之后永远不等」的错误实现照样能让
>    background 那条通过——那条只断言「没等」，而「永远不等」当然也满足「没等」。
>    ⚠ 用「改坏产品代码看它红不红」验判据是**一次性**动作，做完就没了；
>    写成用例，判据的分辨力才会被长期钉住。（一次性那道也做了：
>    把 `record.awaited = not background` 改成恒真，用例当场红。）
>
> #### 唯一的例外：被测的性质本身就是「多久之内返回」
>
> `test_subprocess_timeout.py` 通篇用挂钟时间判，那是**对的**——它验的是
> 「超时是不是真的等待上限」，除了钟没有别的东西可以问。两条与上面不同：
> 判据钉的是**快慢差一个数量级**（同一条命令上正反各跑一次，比的是两者之差），
> 而不是某个精确值；且两侧各留 2 秒以上余量。
>
> 分辨这两类的判据只有一句：**如果换个更快的机器判据就失去意义，那它验的是顺序**
> （该改）；**如果换个更快的机器判据照样成立，那它验的就是时间本身**（可以留）。

### 起 shell 子进程的超时（2026-08-14）

`test_subprocess_timeout.py`（4 条）——`run_command` 与 Hook 的 `command` 动作
原本用 `subprocess.run(shell=True, capture_output=True, timeout=T)`，
而**那个组合下 `timeout` 只改变返回的文案**：超时后 Python 只杀 shell 壳层，
孙子进程仍握着输出管道的写端，`communicate()` 要一直阻塞到它自己跑完。
实测 `timeout=1` 的调用在命令 `sleep 30` 时真的等 30 秒。

⚠ **这层防护的报警器只有这一个文件。** 改回 `subprocess.run` 之后，
既有用例**一条都不会红**——它们断言的是「返回了超时文案」，而那一条改坏了
也成立，唯一会变的只有墙上的钟。因此这里的反证（同一条命令上跑一次朴素写法、
断言它必须慢）不是冗余，删掉等于把整层防护变成无人看守。

另有一条单独验「**进程真的死了**」：返回得快不等于停下来了。它让脚本睡完再落
一个标记文件，超时返回后等过那个时刻再看文件在不在——没有这一条的话，
一个「立刻返回但不杀进程」的实现照样全绿，而被以为终止的命令还在读写文件。

## C16 分类器审查（九个文件，约 130 条）

| 文件 | 钉住什么 |
| --- | --- |
| `test_classifier_prompt.py` | **工具输出与模型正文不进提示词**（反证用真的会起作用的注入样本）、超长内容逐字完整**且源码里不得有截断分支**、两次渲染前缀逐字相同、标记块片段被无害化、取 `content` 而非 `display_content`、两阶段共用同一段判断依据 |
| `test_classifier_parse.py` | 两阶段各自的解析；⚠ **`OnlyOnePathAllowsTest` 遍历一批异常输出，断言没有任何一种能被解析成放行**——没有它，一次「解析器写宽了」的改动会静默把方向从收紧变成放宽 |
| `test_classifier_breaker.py` | 两种熔断、失败**不计入**拦截计数、任一放行重置连续计数；⚠ **`test_counters_are_shared_across_scopes`（三类各拦 1 次即熔断）钉住「不得按类别分桶」**，另有一条结构护栏断言 `record_block` 不接受区分类别的参数 |
| `test_classifier_cache.py` | 只有网络类进缓存、两种有效期不同、键不含路径与查询参数、失败不缓存、命中时耗时归零 |
| `test_classifier_broad.py` | 三类判据（遍历两张常量表，新增项自动覆盖）；⚠ 两条钉「刻意」的反证——**窄规则必须保留**（含 `python -m unittest*`）、**`Bash(git *)` 必须不在清单内**；另有一条 AST 层的叶子包护栏（只看 import 语句，不按全文搜索——否则会因为一段正确的注释而红） |
| `test_classifier_service.py` | ⚠ **第一阶段判不可疑时第二阶段零次调用**（断言次数，不是断言结果）、`tools=None`、失败三形态全部得到 FAILED、绝不外抛、熔断后不再发请求、记录里 `specifier` 是全文不截断 |
| `test_classifier_loop.py` | ⚠ **`allow` 命中时零次调用**、`deny` 命中时零次调用、只读与写文件工具零次调用；网络类**有对照组**（不开分类器时仍弹面板 vs 开了不弹），把「唯一一处可以放宽」这个不对称钉住；熔断后命令/网络退回面板；工作目录如实传入 |
| `test_classifier_message.py` | 拦下时**不投递**、回灌文案写明「没有送达 / 没有被唤醒」、任何情况下不弹面板、熔断后**一律投递**（与另外两类相反）、`deny` 规则零次调用（证明没另开绕过 `engine.decide` 的路径）、严格档下仍经分类器；⚠ **`ArgumentNameTest` 拿三个工具真实声明的 `parameters` 逐个比对**——实现期真踩过 `body` vs `message`，后果完全无声 |
| `test_classifier_config.py` | 三个字段的缺省与显式值；⚠ **两种口径的不对称**：`enabled` 非法**抛错**、`timeout` 非法**回退默认**；`True` 不得被当成 1.0 秒；`request_timeout` **刻意不进模板** |

⚠ **测试设施侧另有一条**：分类器与主对话**共用 `provider_factory`**（绕过它会让
端到端测试静默连真实网络），因此两个剧本 Provider 都据系统提示的首行标记
自己应答分类器请求、**不消耗剧本轮次**。缺省回 `pass` 使既有每一份剧本一字不用改。
不做这件事的话，分类器每判定一次就吃掉一轮主对话剧本——C15 的并行组队剧本
因此整个错位，表现是「一条队友消息都没发出去」，看起来像协作功能坏了。
