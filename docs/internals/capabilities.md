...
- **覆盖哪三类**：`run_command`（命令）、`web_fetch`（网络）、`send_message`（队友消息）。
...
- **三类的落点**：
  ...
  | 分类器 | 命令 | 网络 | 消息 |
  | --- | --- | --- | --- |
  | 放行 | 照常执行 | **不再弹面板**、直接执行 | 照常投递 |
  | 拦下 | 拒绝、不弹面板 | 拒绝、不弹面板 | **不投递**、不弹面板 |
  | 失败（未熔断） | 拒绝 | 拒绝 | 不投递 |
  | 失败（已熔断） | **弹面板** | **弹面板** | **一律投递** |

  ⚠ 网络那一格的「放行」是本章唯一一处**放宽**：它的④层基线本来就是 ASK。
...
- **宽泛放行规则**：启用时从**文件规则集**里丢弃三类命令放行规则
  （整工具通配 / 解释器 + 通配 / 包管理器 run 类）并逐条告知；
  会话级与本次执行级规则不动，域名规则不动。`Bash(git *)` **刻意不在清单内**。
