...
- **覆盖哪四类**：`run_command`（命令）、`web_fetch`（网络）、`send_message`（队友消息）、
  `web_search`（**网络搜索**，web_search 扩展新增）。
  文件的读写查搜、MCP 工具、委派、Skill 加载、共享任务清单**一律不进**。
...
- **四类的落点**：
  ...
  | 分类器 | 命令 | 网络 | 消息 | 搜索 |
  | --- | --- | --- | --- | --- |
  | 放行 | 照常执行 | **不再弹面板**、直接执行 | 照常投递 | **不再弹面板**、直接执行 |
  | 拦下 | 拒绝、不弹面板 | 拒绝、不弹面板 | **不投递**、不弹面板 | 拒绝、不弹面板 |
  | 失败（未熔断） | 拒绝 | 拒绝 | 不投递 | 拒绝 |
  | 失败（已熔断） | **弹面板** | **弹面板** | **一律投递** | **弹面板** |

  ⚠ 网络与搜索那两格的「放行」是**放宽**：它们的④层基线本来就是 ASK。
  搜索类的基线取 ASK 而不是 ALLOW 正是为了让最后一行成立——基线若是 ALLOW，
  熔断后会退回**一律放行**，「查询词外泄」这条防线在故障时完全消失。
