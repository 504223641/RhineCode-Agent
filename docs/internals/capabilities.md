...
- **Skill 体检**（作者期扩展）：`/skills` 报告里的**第四类反馈**（前三类是加载失败 / 警告 / 字段提示，都只说「发生了什么」；这一类说「那我该怎么改」）。纯函数零 IO、**每次现算不缓存**、排在前三类之后、**无建议时整段不出现**、有建议时段尾指向 `/skill-creator`。七项检查与各自阈值：
  ...
  | 3 | 正文无 `$ARGUMENTS` | 建议里**必须**写明「参数不会丢、只是被追加到末尾」，否则把「位置不对」误导成「功能失效」 |
  | 4 | 预授权过宽 | 非 MCP：工具名不在 `READ_ONLY_GRANT_TOOLS`（现仅 `Read`）**且**（无 pattern **或** pattern 是纯通配 `*` / `domain:*`）。**MCP 另判**：按工具名含不含通配符——引擎对 `other` 分支要求 `pattern == ""`，套用前一支不仅每条必误报，改法「加参数模式」还会让预授权**永远匹配不上** |
  | 5 | 预授权全军覆没 | 有声明但翻译出的规则数为 0。**部分生效不报**（单条认不出已有警告，这里报的是整体误解） |
  | 6 | 逼近注入上限 | **渲染后的注入段**（含边界标识、说明行、资源清单）达 `BODY_MAX_*` 的 **80%**（240 行 / 9830 字节）。已被截断时直接命中且措辞区分。渲染**传空参数**，是刻意偏松 |
  | 7 | 覆盖了内置样板 | `(命令名, BUILTIN) in SkillCatalog.shadowed`。**收窄到只报覆盖内置**，项目级盖用户级不报。措辞中性、含「若是有意定制请忽略」 |
