...
新增 Skill（c11）：**不写代码**——在 `<项目根>/.rhinecode/skills/` 或 `~/.rhinecode/skills/` 放一个 `<name>.md`（或一个含 `SKILL.md` 的目录），frontmatter 写 `name`/`description` 两个必填项，按需加 `allowed-tools`/`when_to_use`/`allowed-tools`/`context`/`disable-model-invocation`/`user-invocable`/`model`，正文写 SOP 指令并用 `$ARGUMENTS` 承接用户参数。运行中 `/skills reload` 即可生效。
