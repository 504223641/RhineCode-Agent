...
- **`config._KNOWN_PROVIDERS` ↔ `web/search.PROVIDERS`（web_search）** → 加第二家搜索服务商时两处齐改。**刻意不 import**：让 `config.py` 依赖 `web` 是层级倒挂，且只为拿一个名字清单会把 `web/models.py` 一并拉起来。漏改的表现是「配置里写了新服务商，启动时说不认识」——**会当场报错，不会静默**，所以这条比其余几条温和，但仍然值得记一笔
