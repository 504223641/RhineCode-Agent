...
- **新增一类要经分类器审查的动作（c16）** → **四处齐改**：`classifier/models.py` 的 `SCOPE_*` 常量 + 工具上声明 `classifier_scope`（`tools/base.py`）+ `agent/loop.py` 的 `_review_action` 与**两条判定分支**（普通分支与 `system_serial` 分支）+ `classifier/prompt.py` 的待判动作段落 + `classifier/render.py` 的 `_SCOPE_LABELS`。**漏改后三处不报错**，只表现为「声明了但从不被审查」（或界面上范围名是空白）——而配置和界面上都看不出异常。护栏见 `tests/test_classifier_loop.py`（触发与不触发各有正反例）与 `tests/test_web_search_classifier.py::FourPlacesTests`（**四条分开写，漏改哪处红哪条**）。⚠ **已有两个实例**：C16 原始的三类，与 web_search 扩展新增的搜索类——后者是照着这条清单做的，四处一次到位
