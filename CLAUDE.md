...
python -m unittest discover -s tests      # 3323 项，skipped 4，约 3.5 分钟
...
（那 5 次跑在删除 anthropic / openai 两条 Provider 用例**之前**，所以当时是 3325 条，
不是上面的 3323——这不是笔误）。**改动它或测试夹具之后建议再连跑几次**，
别只跑一次就下结论。

**串行跑满三分多钟是正常的**，且几乎全是「为验真实行为付的真实代价」：2026-08-20 实测
（3323 项、用例内耗时合计 208.5 秒），**94.7% 的时间**花在真起子进程（e2e 宿主 /
`git` / `run_command`）、真跑一个 Textual app 上——而那部分只有 **620 条**用例；
剩下的 **2703 条（81%）纯逻辑用例加起来只有 11.0 秒**。
⚠ **这几个数字是一组，别单独改其中一个**：2026-08-19 之前那次就是只更新了总数
（3115 → 3143），于是「85%」对不上任何一个分子分母。
⚠ **再往前那组（87% / 737 / 2493）本身就对不上**：737 + 2493 = 3230，
而总数写的是 3143——它是「只改了一部分」的又一次现形。
⚠ **上一组（654 / 2662 / 3316）内部自洽，却与仓库对不上**：它是 2026-08-19 量的，
之后有两次改动各加了用例却没回来更新这里，实测时仓库已是 3318 项。
**「三个数字互相对得上」不等于「它们描述的是当前这棵树」**——量之前先跑一次
`discover` 看总数对不对得上，别只做加减法。本次的三个数字**自洽且刚量过**：
620 + 2703 = 3323，请照这个口径维护。
量法可复现：自定义 `TestResult` 在每条用例前后打时间戳、按模块聚合，
把「整个模块总耗时 < 1 秒」的算作纯逻辑。
