...
Skill 系统测试（`tests/test_skill_*.py`，c11，共 11 个文件 210 条）：解析器（六字段与缺省值、未知键向前兼容、allowed_tools 去重保序、名字规则与四个保留子命令词、YAML 布尔裸词的加引号提示、各类失败原因可读）、三层扫描（单文件型与目录型、资源清单排序截断与 POSIX 分隔符、三层同名整份覆盖不做字段合并、覆盖不记 error 且被覆盖那份的警告不发出、层内字典序去重、四种坏样本不阻断其余）、渲染（清单不含正文、`$ARGUMENTS` 全量替换与参数原样保留、TRUNCATED 与 DROPPED 可区分、字节截断不产生非法 UTF-8、自包含调用文本三项俱全）、白名单校验（内置名笔误致命 / `mcp__` 不致命 / 单下划线 `mcp_add_server` 正常通过三分支、剔空降级为不收窄）、编排（幂等激活位置不动、工具策略四分支与运行期自愈、热更新 added/removed/auto_deactivated/dropped_fatal 与「下次启动会失败」文案、项目提示零状态、三个内置样板开箱可见，以及**加锁不变量的跨线程死锁护栏**——回调桩另起 daemon 线程读状态并 `join(timeout)`，用完成计数而非布尔标志，同线程版本在 `RLock` 下会静默通过故不可简化）、`load_skill` 工具（`read_only` 与真实权限引擎判定为 ALLOW、三态输出、成功时不回灌正文）、循环策略（不传 `options` 与 C10 一致、`excluded` 压过 `exempt`、Plan Mode 交白名单与只读、`dynamic` 与 `tool_policy` 每轮求值，以及**跨轮端到端护栏**：第 1 轮调 `load_skill`、断言第 1 轮 reminder 不含 SOP 而第 2 轮含）、命令层（闭包绑定各自正确、重名跳过且不留幽灵索引项、`has_skill_command` 在重名时为 False、`/skills` 五形态分派）、独立模式（主历史恰好两条配对消息、五种停止原因文案、子历史不拆散工具配对、子对话不写存档不更新锚点、`cancel_event` 重建、指定模型换 Provider、只跑 C8 第一层且第二层未被调用）、沙箱（读面扩大而写/glob/grep 面完全不动、`..` 仍拒、危险命令仍被黑名单拦、**走真实 `/resume` 路径**验证激活态清空、失败路径保持不变）、TUI（状态栏 Skill 段、短命令补全与三参数传递、激活通知刷新、提交守卫可见提示且同一忙碌期只提示一次）、启动接线（笔误退出码 1 且 `connect_all` 未被调用、豁免名与单下划线内置工具正常启动、`load_skill` 写进白名单不致启动失败的顺序护栏、重名短命令跳过而 Skill 仍可用、项目提示每次启动都出现）。真实 LLM 下的 Skill 执行质量与 TUI 端到端场景见 `docs/c11/checklist.md` 第十一节，**11 条中的 10 条已用 P1a 驱动设施以真实模型实跑完毕**（43/43 判据全通过、产品代码一行未动，报告见 `docs/c11/acceptance-c11-live.md`，预置见 `tests/e2e/c11_scenarios.py`）。它们**刻意不写成自动化回归测试**——判据多在「真实模型是否按 SOP 行事」，模型行为不确定，今天绿明天红的测试最终一定被 skip 掉；只验一次、留证据。唯一未跑的**场景 9（运行中的输入反馈）是 P1a 结构上驱动不了**：`DriverCore.send` 的前置检查写死「只有 idle 时才能提交」，忙碌态下提交递不进产品，被拦的是驱动器自己的守卫；该行为已由 `tests/test_skill_tui.py` 的 Pilot 用例覆盖，控制通道的「强制提交」指令推给 P1b。
...
2. Plan Mode 规划阶段的工具阶段强校验，防止模型同轮夹带副作用工具。**已有真实观测样本**（C11 场景 10 验收时撞到，见 `docs/c11/acceptance-c11-live.md`）：规划阶段那一轮的 `tool_names` 里没有 `run_command`（`_schema_for` 用 `readonly_schemas()` 滤掉了它），模型仍凭先验调了出来，而 `_execute` 的 `out_of_scope` 判据只查 **Skill 白名单**（`_visible(name, policy)`）、不查规划阶段的只读过滤，于是 `outcome=executed`。**这不是 C11 `out_of_scope` 守卫漏了**——两处过滤职责不同。缓解是五层管线一层没少（照样弹确认面板）；这次夹带的恰好是 `git diff`，但同一路径上完全可能是写命令。
