...
| 章节 | 能力 | 一句话 |
| --- | --- | --- |
| C4 | Agent Loop 与 Plan Mode | ReAct 循环：调模型 → 执行工具 → 回灌结果 → 再调模型，直到自然完成或命中停止条件。⚠ **2026-09-18 起循环不再有轮次硬顶**：`MAX_ITERATIONS = 25` 变成**检查点间隔**，到了那条线由一次独立的模型调用（`classifier/continuation.py`，**不是** c16 那个安全分类器，两者只共用 Provider）回答「它还在往前走吗」，判「继续」就再给同样多的轮次；连续放行有上限（约 125 轮）、问不出来一律停、没配判定器时行为逐字不变。同时停止条件里多了「原地打转」**（`agent/spinning.py`：同一个 `(工具名, 参数, 结果)` 重复 3 次即停，纯代码零成本）——它把原先由 25 轮上限一个数字同时承担的两件事拆开了：「卡住了」由它判，「任务复杂还要不要接着跑」是另一条路。上游两家的主循环**都不设轮次上限**（Claude Code 的 `maxTurns` 可选且无默认值，Codex 二进制里搜不到任何轮次上限、改用 token 预算 + 提示词里的 no-progress check） |
...
| C8 | 上下文管理（LLM 摘要） | 每次请求前「锚点 + 增量」估算用量；**逼近窗口时**调 LLM 把早段压成结构化摘要、近期原文留下。fail-safe、连续失败 3 次熔断，用户原始消息永不改写。⚠ **2026-09-17 删掉了原来的「第一层·工具结果存盘」**：它按绝对阈值触发（合计 16000 token），而那个数定于「DeepSeek 约 64K 窗口」的年代——窗口换成 1M 之后它相当于**用量到 1.6% 就开始删历史**，真实 trace 实录里模型因此连续十几轮拿不到自己刚读的文件、最终撞迭代上限、一个文件都没写出来。对齐上游：Claude Code 与 Codex **都没有「事后删历史」这种机制**，一律在工具**产出的那一刻**限量（现在由 `run_command.RUN_OUTPUT_MAX_CHARS` / `read_file.READ_OUTPUT_MAX_CHARS` 承担），进了历史就不再动 |
...
另有一套**跨阶段的测试设施**（不占章节号、缺省关闭、不进产品包）：**Trace 行为记录器**（`--trace`）把运行过程写成三十二类结构化事件的 JSONL 配只读阅读器；**端到端驱动设施**（`tests/e2e/`）起常驻宿主让 Claude 经本机回环通道自己驱动界面跑完整交互闭环。两者都用于验收既有能力与排查那类「界面上看不出、但行为确实不对」的问题。

⚠️ **术语提醒：用户口中的「调度器」一律指上面这套端到端驱动设施**（`tests/e2e/`
的 `host` / `client` / `control` / `protocol` 四件），也就是「看 trace + 模仿真人
操作界面来测 RhineCode」的那个东西。**项目里没有任何组件叫这个名字**，所以很容易
猜错成另外两个「也在调度」的地方——`agent/loop.py` 的工具调度（只读并发桶 / 副作用
串行桶）与 `subagents/tasks.py` + `team/` 的任务调度（委派、交付、待命唤醒）。
它与 trace 是**配对使用**的：trace 记下「发生了什么」，调度器让「那件事发生」。
...
| Context | `context/` | 估算与 LLM 摘要 | `allow_summary` 必须在 `and` 链最前面短路——锚点对应主历史，拿它估子对话毫无意义。**第二条**：这一层现在是「进了历史就不动，直到逼近窗口」，别再往里加「平时就删点什么」的机制——那正是 2026-09-17 删掉第一层的理由 |
...
- 上下文管理（c8）：⚠ **第一层存盘已于 2026-09-17 删除**，不再产生新的 `.rhinecode/context/*.txt`；但**旧文件仍躺在每个跑过 rhine 的项目里**，它们是工具结果原文、可能含被读过的敏感文件片段，故 `.rhinecode/context/` 仍应 git 忽略、勿提交（已在 `.gitignore`），`path_guard` 的搜索跳过表与 `permission/protected.py` 的排除清单里那两条也**刻意留着**（两处都写了理由，别当成漏删）。摘要会把「较早的对话历史（含工具结果）」作为一条 user 转录发给 LLM——与正常对话同样是把上下文交给模型，无额外外泄面，但若用了 `deny Read(敏感文件)`，注意该文件内容一旦已进入历史仍可能被摘要带走（deny 只挡新读取，挡不住已在历史里的内容）。摘要请求强制 `tools=None`，模型在摘要阶段无法调用任何工具。
...
- **#22** `retain_budget` 在大窗口下留得太少（1M 窗口下一次摘要只保留最近 1% 的原文）。与 #8 同形态、当时只修了小窗口那一端。**改动会直接改变摘要的内容质量，需要真实模型验收**，故单独立项；⚠ 它必须与触发线一起定，单改一个会把摘要频率带偏。

已修复/已推翻的五条（2、12、17、18、21）连同它们的理由与护栏位置，都留在那份文档里。
⚠ **#21 值得顺手看一眼**（2026-09-17，c8 存盘占位把模型指向存盘文件、形成一个
**不收敛**的死循环，真实 trace 里 36% 的迭代白烧）：它记的不只是那个 bug，还有一条
可复用的教训——**同一个「自放大」环在搜索那一侧早就修过，读取这一侧当时没覆盖到**，
而后者更糟（搜索是偶然命中，读取是占位符主动指过去的）。修掉一个自放大环之后，
要问一遍「同一个环还有没有别的入口」。

⚠ **它还有第二段，同样值得看**：修完 #21 之后**同一条任务照样撞满 25 轮、照样
零产出**——因为那个 bug 只是症状之一，病根是**第一层存盘本身**（按绝对阈值触发，
1M 窗口下相当于用量到 1.6% 就开始删历史，而被删的有一半是模型**还没看过**的）。
第一层已于同日整层删除。教训：**修掉一个具体的 bug 之后，要再问一遍「它所在的
那个机制本身是不是也该没有」**——上游两家（Claude Code / Codex）都没有这一层。
