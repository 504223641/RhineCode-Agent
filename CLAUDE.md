...
python -m unittest discover -s tests      # 3316 项，skipped 4，约 4 分钟
```

**跑满几分钟是正常的**，且几乎全是「为验真实行为付的真实代价」：2026-08-19 实测
（3316 项、用例内耗时合计 243.6 秒），**96% 的时间**花在真起子进程（e2e 宿主 /
`git` / `run_command`）、真跑一个 Textual app 上——而那部分只有 **654 条**用例；
剩下的 **2662 条（80%）纯逻辑用例加起来只有 10.6 秒**。
⚠ **这几个数字是一组，别单独改其中一个**：上上次就是只更新了总数
（3115 → 3143），于是「85%」对不上任何一个分子分母。
⚠ **上一组数字（87% / 737 / 2493）本身就对不上**：737 + 2493 = 3230，
而总数写的是 3143——它是「只改了一部分」的又一次现形。本次的三个数字**自洽**：
654 + 2662 = 3316，请照这个口径维护。
