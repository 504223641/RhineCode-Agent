...
python -m unittest discover -s tests      # 3143 项，skipped 4，约 4 分钟
```

**跑满几分钟是正常的**，且几乎全是「为验真实行为付的真实代价」：2026-08-19 实测
（3143 项、用例内耗时合计 229 秒），**87% 的时间**花在真起子进程（e2e 宿主 /
`git` / `run_command`）、真跑一个 Textual app 上——而那部分只有 **737 条**用例；
剩下的 **2493 条（79%）纯逻辑用例加起来只有 9.7 秒**。
⚠ **这几个数字是一组，别单独改其中一个**：上一次就是只更新了上面那行的总数
（3115 → 3143），于是「85%」对不上任何一个分子分母（它是总数还是 2645 时算的）。
