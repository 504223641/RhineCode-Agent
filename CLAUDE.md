...
4. **权限模式**：严格/默认/放行，只兜底「规则未命中」的灰色地带，翻不了①②③的 deny。⚠ **auto-plan 扩展起，主对话的启动档是「放行」**（`auto` 预设的档位），且**运行期不可切换**（`/perm` 已删除）。因此**缺省体验是「工作区内的文件写入与命令执行都不弹面板」**——此前缺省是「默认档、灰色地带交人工确认」。缺省下仍然会弹面板的只剩四类：**②″保护路径**的写入、**网络访问**（未建域名白名单时）、**`mcp_add_server`**（`launch` 类，B4 修复后新增——它会写一条 `mcpServers` 配置并立刻拉起一个**任意本地命令**，而①②②′②″没有任何一层碰得到它）、以及用户自己写的 Hook `ask` 规则。⚠ **MCP Server 提供的工具**（`mcp__<server>__<tool>`）**不在这四类里**——它们落 `other` 分支，缺省预设下是 `allow @ mode`，即**零提示零面板**；要逐次确认得自己写 `deny` 规则或收紧角色的 `permission_mode`。要更严只能写 `permissions.yaml` 的 `deny` 规则，或给子 Agent 角色声明更严的 `permission_mode`。
...
- MCP 工具（c7）：远端 Server 是外部程序、不可信，故 MCP 工具一律 `read_only=False`。⚠ **这里一度写着「默认权限模式下每次调用都经人在回路确认」，那句话已经不成立**（审查报告 B5）：它说的是 `PermissionMode.DEFAULT`，而 **`DEFAULT` 早就不是缺省了**——auto-plan 扩展把缺省预设定成 `auto`，其档位是 `PERMISSIVE`。**实情是：缺省预设 `auto` 下 MCP 工具直接放行**（实测 `mcp__everything__printEnv` 是 `allow @ mode`），零提示、零面板。要逐次确认，只能写 `deny` 规则（⚠ 整工具形式，`deny: mcp__server__*` 这类通配可用）、或给子 Agent 角色声明更严的 `permission_mode`。`kind="other"` 会跳过①黑名单与②沙箱（它们针对本地命令/路径），但仍走③规则（`allow: mcp__server__*` 可放行）与④模式兜底。若工具名被规范化，规则要写注册名而不是远端原名。stdio Server 的子进程行为不受路径沙箱约束（与 `run_command` 同属已知边界）；`mcp_add_server` 会写配置并启动外部 MCP，因此它**不落 `other` 分支**——adapter 把它映射成 `launch` 类，第④层在放行档下对它判 ASK（B4 修复；此前它与其余 MCP 工具一样是 `allow @ mode`，六层防御一层都不生效）。⚠ 它的 stdio 子进程**不再继承含密钥的完整环境**（复用 `run_command` 那份过滤），但 `mcp.yaml` 里显式写下的 `env` 仍照常送达；`mcp.yaml` 的 `env`/`headers` 可能含密钥（如 `${API_KEY}`），同样勿提交真实值，也不要让自动解析逻辑生成真实密钥。
...
- **命令与网络的分类器审查（c16）七条**：
  ...
  **MCP 工具**不进（它们不落这三类中的任何一类）。另有**两条方向相反**的已知边界，是同一个机制（每次从对话里重读用户消息）的两面：
