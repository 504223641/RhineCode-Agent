...
python -m unittest discover -s tests      # 3325 项，skipped 4，约 4 分钟
```

**跑满几分钟是正常的**，且几乎全是「为验真实行为付的真实代价」：2026-08-20 实测
（3325 项、用例内耗时合计 236.5 秒），**96% 的时间**花在真起子进程（e2e 宿主 /
`git` / `run_command`）、真跑一个 Textual app 上——而那部分只有 **657 条**用例；
剩下的 **2668 条（80%）纯逻辑用例加起来只有 10.5 秒**。
...
657 + 2668 = 3325，请照这个口径维护。
⚠ **2026-08-20 的分类器误伤修复新增 4 条纯逻辑用例**，三个数字按口径同步过：
跑了一次 `discover` 确认总数确实是 3325（不是只做加减法），新增的四条都在
`test_classifier_prompt.py`（整模块耗时远小于 1 秒），故只动纯逻辑那一格。
