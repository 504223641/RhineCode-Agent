...
| 章节 | 能力 | 一句话 |
| --- | --- | --- |
| C4 | Agent Loop 与 Plan Mode | ReAct 循环：调模型 → 执行工具 → 回灌结果 → 再调模型，直到自然完成或命中停止条件。⚠ **2026-09-18 起停止条件里多了「原地打转」**（`agent/spinning.py`：同一个 `(工具名, 参数, 结果)` 重复 3 次即停，纯代码零成本）——它把原先由 25 轮上限一个数字同时承担的两件事拆开了：「卡住了」由它判，「任务复杂还要不要接着跑」是另一条路。上游两家的主循环**都不设轮次上限**（Claude Code 的 `maxTurns` 可选且无默认值，Codex 二进制里搜不到任何轮次上限、改用 token 预算 + 提示词里的 no-progress check） |
