...
- **命令与网络的分类器审查（c16）七条**：
  ...
  **⑦b 反过来：早先消息里的敏感内容会一直污染后续判定，且不随时间衰减。**
  用户只要在对话里提过一次密钥样式的字符串，之后**完全无关**的动作也可能被
  以「用户曾提到过那个密钥」为由拦下。⚠ **2026-08-20 实测撞到**（一份真实
  trace）：用户先问「搜下 sk-proj-abc123」，接着问「搜下今天的 AI 新闻」——
  后者被拦，理由写的是前者。**待判动作里根本没有密钥。** 分类器把
  「对话里出现过」当成了「这个动作会泄漏」。
  ⚠ 同一轮里两条语义相同的查询**一条放行一条拦下**（中文过、英文没过），
  说明它对措辞敏感。用户侧唯一的解法是 `/clear`（把那条消息从转录里去掉）。
