...
python -m unittest discover -s tests      # 3297 项，skipped 4，约 4 分钟
```

**跑满几分钟是正常的**，且几乎全是「为验真实行为付的真实代价」：2026-08-19 实测
（3297 项、用例内耗时合计 234.5 秒），**96% 的时间**花在真起子进程（e2e 宿主 /
`git` / `run_command`）、真跑一个 Textual app 上——而那部分只有 **679 条**用例；
剩下的 **2618 条（79%）纯逻辑用例加起来只有 9.6 秒**。
