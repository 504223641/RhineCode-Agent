...
# 只读检索里**有一个失败**，而模型不停下来继续跑（原 AC9 的兜底路径）。
# 真正要紧的失败会让模型停下来说明，那种情况批次自然断开、那条调用单独可见；
# 这里覆盖的是「并行调用中某个失败、模型没停」。
#
# ⚠ **手测时的预期已于 2026-09-17 反转**（tui-activity-fold 原 F6，见该 spec 的
# 勘误块）：折叠行**保持绿色、不提失败**，要按一次 `Ctrl+O` 才看到那条失败的
# 工具行（红 + 「失败」二字）。照旧预期看会以为是产品坏了。
FOLD_WITH_FAILURE = [
    [
        text("我查几个地方。"),
        tool("grep_content", {"pattern": "def "}),
