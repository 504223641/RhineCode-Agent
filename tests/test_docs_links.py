...
## ⚠ 一处刻意接受的盲区：它看不见「第 N 条」

规则只认 `N-slug.md`，所以正文里「与**第 1 条**同源」这种写法**它一声都不吭**。
2026-08-31 那次重排实测：护栏一次报出 **16 处**（6 个文件，其中 4 个在 `docs/todo/`
之外），全部机械修掉；而**另外两处「第 N 条」是人工翻出来的**。

**不去匹配它是刻意的**：「第 N 条」在本仓库里到处都是（spec 条款、安全边界第 N 条、
已知项第 N 条……），匹配它必然大量误报——而**一个会误报的门禁的结局是被整体关掉**，
那样连现在能查出来的 16 处也一起没了。

补救不在代码里，在 `docs/todo/README.md` 的重排说明里：多跑一条
`grep -rn "第 [0-9] 条" docs/todo/` 并**逐条人看**（那里大半出现在历史重排记录与
「⚠ 上一版这里写的是……」里，引用的是**当时**的编号，是对的，机械替换反而会改错）。
"""
